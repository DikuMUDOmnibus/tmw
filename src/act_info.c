...
{
    "&B<&Yu&Ws&Ye&Wd &Ya&Ws l&Yi&Wght&B>     ",
    "&B<&zworn on finger&B>    ",
    "&B<&zworn on finger&B>    ",
    "&B<&zworn around neck&B>  ",
    "&B<&zworn around neck&B>  ",
    "&B<&zworn on body&B>      ",
    "&B<&zworn on head&B>      ",
    "&B<&zworn in eyes&B>      ",
    ...
    "&B<&zworn on arms&B>      ",
    "&B<&zworn as shield&B>    ",
    "&B<&zworn about body&B>   ",
    "&B<&zworn about waist&B>  ",
    "&B<&zworn around wrist&B> ",
    "&B<&zworn around wrist&B> ",
    "&B<&RW&ri&We&zl&Wd&re&Rd&B>           ",
    "<&RD&ru&Wa&zl Wie&Wld&re&Rd&B>      ",
    "&B<&Ph&pe&Pl&pd&B>              ",
    "&B<&zworn on ear&B>       ",
    "&B<&zworn on ear&B>       ",
    "&B<&Pw&porn &Pa&pround &Pa&pnkle&B> ",
    "&B<&Pw&porn &Pa&pround &Pa&pnkle&B> ",
    "&B<&RT&rr&Wip&zle wi&Wel&rde&Rd&B>    "
};

char *  const mech_where [] =
{
 "&W<head>         ",
 "&W<center torso> ",
 "&W<left torso>   ",
...
void show_char_to_char_2  args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    buf[0] = '\0';
    if ( IS_OBJ_STAT( obj, ITEM_INVIS)     )   strcat( buf, "&z(&WInvis&z) "     );
    if ( IS_AFFECTED( ch, AFF_DETECT_EVIL  )
	&& IS_OBJ_STAT( obj, ITEM_EVIL )   )   strcat( buf, "&r(&RRed Aura&r)&w "  );
    if ( IS_AFFECTED( ch, AFF_DETECT_MAGIC )
	&& IS_OBJ_STAT( obj, ITEM_MAGIC )  )   strcat( buf, "&Y(&BMagical&Y)&w "   );
    if ( IS_OBJ_STAT( obj, ITEM_GLOW )     )   strcat( buf, "&W(G&zl&Bo&bw&Bi&zn&Wg)&B "   );
    if ( IS_OBJ_STAT( obj, ITEM_HUM )      )   strcat( buf, "&C(&PH&pu&Cmmi&pn&Pg&C) "   );
    if ( IS_OBJ_STAT( obj, ITEM_POISONED ) )   strcat( buf, "&G(&gP&Goisone&gd&G)&w "  );
    if ( IS_OBJ_STAT( obj, ITEM_FLAME )    )   strcat( buf, "&r(&RB&ru&Orni&rn&Rg&r)&w " );
    if ( IS_OBJ_STAT( obj, ITEM_CHAOS )    )   strcat( buf, "&Y(&OC&Yhaoti&Oc&Y)&w " );
    if ( IS_OBJ_STAT( obj, ITEM_ICY   )    )   strcat( buf, "&B(&CF&cr&Co&cs&Ct&cy&B)&w " );
    if ( fShort )
    {
	if ( obj->short_descr )
...
void show_list_to_char( OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing )
{
	...
	if ( obj->wear_loc == WEAR_NONE && can_see_obj( ch, obj )
          && !IS_SET(obj->extra_flags, ITEM_LOADED)
		  && !IS_SET(obj->extra_flags, ITEM_BIONIC_EQPD))
       	{
...
/*            act(AT_WHITE, "$p contains:", ch, obj, NULL, TO_CHAR ); */
    ...
    found = FALSE;
    if (check_for_worn( victim, 2457 ))
    {
        send_to_char( AT_GREY, "You see nothing but a shadowy figure.\n\r", ch );
        return;
       }
...
void show_char_to_char_2( CHAR_DATA *ch, CHAR_DATA *victim )
{
 ...
 if( !IS_NPC(victim) )
 {
  sprintf(buf, "%s%s%s%s is here.\n\r",
	 IS_SET(victim->act, PLR_AFK) ? "&z[&PA&z] " : "",
	 IS_SET(victim->act, PLR_QUEST) ? "&z[&YQ&z] " : "",
	 IS_SET(victim->act, PLR_QUESTOR) ? "&z[&YQ&z] " : "",
	 victim->pcdata->in_mech );

  send_to_char(C_DEFAULT,buf,ch);
  sprintf(buf2, "  ...You see %s inside.\n\r", victim->name);
  send_to_char(C_DEFAULT,buf2,ch);
 }
 return;
}
...
/* Thalador room color change */
	...
	break;
	  ...
	  if ( obj->driver == NULL &&
               ...
               obj->passengers > 1 )
	    {
	      send_to_char(C_DEFAULT, "The vehicle is empty.\n\r", ch);
              return;
            }
	  ...
	  for ( rch = ch->in_room->people; rch; rch = rch->next_in_room )
		...
		continue;
              ...
              if ( rch->in_vehicle == obj )
		{
		  if ( obj->driver == rch )
                    {
		      sprintf( new_buf, "Driving: %s\n", rch->name );
		    send_to_char(C_DEFAULT, new_buf, ch );
		    }
		  else if (obj->gunner == rch)
		    {
		      sprintf( new_buf,  "Gunner: %s\n", rch->name );
		      send_to_char(C_DEFAULT, new_buf, ch );
		    }
		  else
		    {
		      sprintf( new_buf, "Passenger: %s\n", rch->name );
		      send_to_char(C_DEFAULT, new_buf, ch );
		    }
		}
	    }
...
void do_exits( CHAR_DATA *ch, char *argument )
{
		...
		!str_cmp( argument, "auto"))
	{
...

		{
				if ( IS_SET (pexit->exit_info, EX_ISWALL ) ||
					IS_SET (pexit->exit_info, EX_CLOSED ) ||
					IS_SET (pexit->exit_info, EX_ISWINDOW ))
				continue;
			found = TRUE;
			if ( fAuto )
			{	
	...
	else
	{
		if ( !( obj = get_obj_here( ch, argument ) ) )
		  {
		   act(AT_WHITE, "You cannot exit that.", ch, NULL, NULL, TO_CHAR );
		   return;
		  }
		if ( obj->item_type != ITEM_VEHICLE )
		  {
		   send_to_char(AT_WHITE, "I hope you're not inside that!\n\r", ch );
		   return;
		  }
		if ( obj->value[7] > 0 )
		{
			...
			ch->in_vehicle == obj )
		{
			...
			if ( obj->gunner == ch )
				obj->gunner = NULL;
		 act(C_DEFAULT,"You exit the $p.",ch,obj,NULL,TO_CHAR);
		 act(C_DEFAULT,"$n steps out of the $p.",ch,obj,NULL,TO_ROOM);
		}
		return;
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
         "&BRace: &r %s  &B Age:&r %d&w(&r%dhrs&w)&B  Vocation:&r%s\n\r",
         (get_race_data(ch->race))->race_full, get_age( ch ),
         (get_age( ch ) - 17 ) * 4 , class_long( ch ) );   
    send_to_char( AT_CYAN, buf, ch );

    sprintf( buf,
         "&BLevel:&r %d  &B Alignment:&r %d\n\r",
         ch->level,
         ch->alignment );
    send_to_char( AT_CYAN, buf, ch );  
    ...
    sprintf( buf,
         "&BStr:&w%d&B(&r%d&B)  &BAutoExit :&w(%s&w)  &BHit Pts:   &w%d&G/&r%d\n\r",
         IS_NPC(ch) ? 13: ch->pcdata->perm_str, IS_NPC(ch) ? 13:
         get_curr_str( ch ),
    ...
    sprintf( buf,
         "&BInt:&w%d&B(&r%d&B)  &BAutoLoot :&w(%s&w)  &BWill Power:&w%d&G/&r%d\n\r",
    ...
    sprintf( buf,
          "&BWis:&w%d&B(&r%d&B)  &BAutoSac  :&w(%s&w)  &BMove Pts:  &w%d&G/&r%d\n\r",
    ...
    sprintf( buf,
          "&BDex:&w%d&B(&r%d&B)  &BAutoCoins:&w(%s&w)  \n\r",
    ...
    sprintf( buf,
         "&BCon:&w%d&B(&r%d&B)  &BAutoSplit:&w(%s&w)\n\r",
    ...
    send_to_char( D,"&B==========================================\n\r", ch);
    send_to_char( AT_BLUE, "Class levels:\n\r", ch );
    sprintf( buf, "&BOpr: &w%d &BMdc: &w%d &BRog: &w%d &BSdr: &w%d &BWth: &w%d\n\r",
    ch->class_level[0], ch->class_level[1], ch->class_level[2], ch->class_level[3],
    ch->class_level[4] );
    send_to_char( C_DEFAULT, buf, ch);
    sprintf( buf, "&BAgt: &w%d &BPil: &w%d &BHck: &w%d &BTch: &w%d &BVam: &w%d\n\r",
    ch->class_level[5], ch->class_level[6], ch->class_level[7],
    ch->class_level[8], ch->class_level[9] );
    send_to_char( C_DEFAULT, buf, ch );
    send_to_char( D,"&B==========================================\n\r", ch);
    sprintf( buf,
         "&BItems:   &r%d&G/&w(&B%d&w)      &BWeight:&r%d&G/&w(&B%d&w)\n\r",
         ch->carry_number, can_carry_n( ch ),
         ch->carry_weight, can_carry_w( ch ) );
    send_to_char(AT_CYAN, buf, ch );

    sprintf( buf,
         "&BWar Pts: &w%d  &BArenaW: &w%d  &BArenaL: &w%d  &BPkills: &w%d &BPkilled: &w%d\n\r",
         ch->warpts, ch->arenawon, ch->arenalost,
...
#endif
    send_to_char( D,"&B==========================================\n\r", ch);
  /* you do not have psition set up correctly 
...
*/
    ...
    sprintf( buf, "Wimpy set to&w %d&B hit points.\n\r", ch->wimpy );
    send_to_char( AT_BLUE, buf, ch );
    if ( !IS_NPC( ch ) )
    {
...
/*  Edited out by Torann
 ...
 if (ch->level >= 12)
     {
          sprintf( buf, "&BHitroll&R %d &B Damroll&R %d \n\r", GET_HITROLL( ch ), GET_DAMROLL( ch ) );
     send_to_char( AT_CYAN, buf, ch);
     } 
    if ( ch->level >= 20 )
    {
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	    "It is %d o'clock %s on %s, %d%s the Month of %s.\n\r",
    ...
    send_to_char(AT_YELLOW, buf, ch );
    sprintf( buf,
	    "The system started on  %s\rThe system time is %s\r",
	    str_boot_time,
...
if (IS_IMMORTAL( ch ))
{
    ...
    {
      sprintf( buf, "MUD compiled at %s\r", (char*)ctime(&statis.st_mtime ) );
      send_to_char( AT_RED, buf, ch );
    }
    else
    {
      if IS_IMMORTAL( ch )
       send_to_char( AT_RED, "No executable available!!\r\n", ch );
    }
}
    return;
}
 ...
 */
/*
 * Newer 'who' command by YOU KNOW WHO ... Don't you?
 */
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    char             buf2     [ MAX_STRING_LENGTH   ];
    char	spacing[MAX_STRING_LENGTH];
    char	tailspace[MAX_STRING_LENGTH];
    CLAN_DATA *      tClan;
    int              iClass;
    int              iLevelLower;
    int              iLevelUpper;   
...
ch);
    send_to_char( C_DEFAULT,
"&z==&G=&z======   &GMachine &zWorld&G: Generations   &z===&G=&z========&G==&z======&G=&z====\n\r" , ch );
    send_to_char( C_DEFAULT,
"&z===&G=&z========&G==&z===========================&G=&z========================\n\r",
ch);
    send_to_char( C_DEFAULT, "\n\r" , ch );
...
"/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\n\r", ch );  
	 ...
	 */
	if ( d->connected != CON_PLAYING || !is_who( ch, wch ) )
	    continue;
...
guild_table[iGuild].name ) )   */
	...
	if ( wch->level >= LEVEL_HERO )
	    switch ( wch->level )
	      {
	      default: break;
              case L_IMP: class = "&zImplement&Oo&zr"; break;
              case L_CON: class = "&cCoder"; break;
	      case L_DIR: class = "&RGuardian"; break;
              case L_GOD: class = "&rBuilder"; break;
	      case L_SEN: class = "&gBuilder"; break;
              case L_DEM: class = "&pBuilder";break;
              case L_JUN: class = "&OBuilder"; break;
	      case L_APP: class = "&bWarlord"; break;
	      case L_CHAMP5:
	      case L_CHAMP4:
	      case L_CHAMP3:
	      case L_CHAMP2:
	      case L_CHAMP1:
	      case LEVEL_HERO: class = "&rCHAMPION"; break;
	      }
	if ( wch->level >= L_APP 
	&& wch->pcdata->whotype 
           ...
           if ( IS_SET(pClan->settings, CLAN_PKILL) )
              switch ( wch->clev )
              {
               ...
               case 0:
                  sprintf( clan, "-<%s>-", pClan->name ); break;
               case 1:
                  sprintf( clan, "-<Enforcer of %s>-", pClan->name ); break;
               case 2:
              ...
              switch ( wch->clev )
              {
               ...
               case 2:
                  sprintf( clan, "(&GCouncil &zof %s&C)", pClan->name ); break;
               case 3:
                  sprintf( clan, "(&GLeader &zof %s&C)", pClan->name ); break;
               case 4:
                  sprintf( clan, "(&GGuardian of %s&C)", pClan->name ); break;
               case 5:
	 ...
	 */
 sprintf(spacing," ");
 if(class_long(wch) == "Techmage")  sprintf(spacing,"   ");
 if(class_long(wch) == "Healer")  sprintf(spacing,"     ");
 if(class_long(wch) == "Spy")  sprintf(spacing,"        ");
 if(class_long(wch) == "Soldier")  sprintf(spacing,"    ");
 if(class_long(wch) == "Psionicist")  sprintf(spacing," ");
 if(class_long(wch) == "Cyborg")  sprintf(spacing,"     ");
 if(class_long(wch) == "Pilot")  sprintf(spacing,"      ");
 if(class_long(wch) == "Genemaster")  sprintf(spacing," ");
 if(class_long(wch) == "Vampire")  sprintf(spacing,"    ");
 if(wch->level < 10)  sprintf(tailspace,"  ");
 else if(wch->level < 100)  sprintf(tailspace," ");
 
	if ( wch->level < LEVEL_HERO )
	  {
	   sprintf( buf + strlen(buf), "[%-11s%s  Lvl %d%s] ",
	   class, spacing, wch->level, tailspace);
	  }
...
send_to_char(C_DEFAULT,
"&z==&G=&z=========&G==&z===========&G=&z=======&G=&z==&G=&z======&G==&z========&G=&z========&G=&z===\n\r \n\r",ch);

    sprintf( buf, "&G[&z%d&G]&z &GImm&zo&Grtals&z.\n\r", num_of_imm );
    sprintf( buf2, "&z[&G%d&z]&G &zTot&Ga&zl Player%s&G.\n\r",
	    nMatch, nMatch == 1 ? "" : "s" );
    strcat( buf, buf2 );
    send_to_char(AT_GREEN, buf, ch );  
	if ( doubleexp() == TRUE) {
		sprintf( buf2, "Double experience is &YON!\n\r" );
...
void do_report( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !is_class( ch, CLASS_VAMPIRE ) )
        sprintf( buf,
	       "You report: %d/%d hp %d/%d wp %d/%d mv %d xp.\n\r",
	       ch->hit,  MAX_HIT(ch),
	       ch->mana, MAX_MANA(ch),
	       ch->move, MAX_MOVE(ch),
	       ch->exp );
    ...
    if ( !is_class( ch, CLASS_VAMPIRE ) )
         sprintf( buf,
	            "$n reports: %d/%d hp %d/%d wp %d/%d mv %d xp.",
        	    ch->hit,  MAX_HIT(ch),
        	    ch->mana, MAX_MANA(ch),
         	    ch->move, MAX_MOVE(ch),
        	    ch->exp );
...
void do_learn( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; skill_table[sn].name[0] != '\0'; sn++ )
	{
		...
		continue;
    	sklvl = (skill_table[sn].skill_level[ch->class[0]]);
		if ( sklvl < 1 || sklvl >= L_APP )
        continue;
		...
		sprintf( buf, "&W%21s &Y%3d",
			skill_table[sn].name, skill_table[sn].cost );
		strcat( buf1, buf );
		if ( ++col % 3 == 0 )
		    strcat( buf1, "\n\r" );
		else
	...
	if ( ch->practice < skill_table[sn].cost )
	{
	    send_to_char(C_DEFAULT, "You do not have enough practices.\n\r", ch );
	    return;
	}

	sklvl = (skill_table[sn].skill_level[ch->class[0]]);

	if ( sn < 0 ||  sklvl < 1 || sklvl >= L_APP )
	{
	    send_to_char(C_DEFAULT, "You can't learn that.\n\r", ch );
	    return;
	}
		if ( skill_table[sn].spell_fun != spell_null )
	{
        ...
        {
            sprintf( buf, "&W%s tells you &c'&CYou've already learned %s&c'&w.\n\r",
                   capitalize( mob->name ), skill_table[sn].name );
            send_to_char( C_DEFAULT, buf, ch );
            sprintf( buf, "&W%s tells you &c'&CTry practicing for more experience&c'&w.\n\r",
                   capitalize( mob->name ) );
            send_to_char( C_DEFAULT, buf, ch );
            return;
        }
     cost = skill_table[sn].cost;
     ch->practice = ch->practice - cost;
/*	    ch->practice--;  COST HERE */
	    ch->pcdata->learned[sn] += int_app[get_curr_int( ch )].learn; 
		act(C_DEFAULT, "You learn $T.",
		    ch, NULL, skill_table[sn].name, TO_CHAR );
		act(C_DEFAULT, "$n learns $T.",
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	if (!can_use_skpell( ch, sn ) )
	{
	send_to_char(C_DEFAULT, "You have not learned this skill yet.\n\r", ch );
	send_to_char(C_DEFAULT, "Try using learn.\n\r", ch );
	return;
	}
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(AT_RED,  IS_SET( ch->act, PLR_COMBAT )
        ...
        send_to_char(AT_RED,  IS_SET( ch->act2, PLR_AUTOASSIST    )
            ? "&w[&Y+AUTOASSIST    &w]&W You will now assist when needed.\n\r"
            : "&w[&Y-autoassist    &w]&W You start your own fights.\n\r"
            , ch );

        send_to_char(AT_RED,  IS_SET( ch->act2, PLR_NOSUMMON    )
            ? "&w[&Y+NOSUMMON    &w]&W You cannot be summoned.\n\r"
            : "&w[&Y-nosummon    &w]&W You are summonable.\n\r"
            , ch );

        send_to_char(AT_RED,  IS_SET( ch->act2, PLR_NOFOLLOW    )
            ? "&w[&Y+NOFOLLOW    &w]&W You do not accept followers.\n\r"
            : "&w[&Y-nofollow    &w]&W You can be followed.\n\r"
            , ch ); 
...
/*	char buf [ MAX_STRING_LENGTH ];
	...
	else
	{
       ...
       if ( fSet )
        {
           if ( !str_cmp( arg+1, "autoassist") ||
                !str_cmp( arg+1, "nofollow") ||
                !str_cmp( arg+1, "nosummon"))
           {
            SET_BIT    ( ch->act2, bit );
...
/*	if (bit == PLR_AUTOASSIST || bit == PLR_NOSUMMON ||
	   bit == PLR_NOFOLLOW )
	{ 
      	  if ( fSet )
       	   {
...
void do_slist ( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( AT_BLUE, "          &CSpells/Skills\n\r\n\r", ch );
...
void do_autoassist ( CHAR_DATA *ch, char *argument )
{
    ...
    ( IS_SET ( ch->act2, PLR_AUTOASSIST )
     ? sprintf( buf, "-autoassist" )
     : sprintf( buf, "+autoassist" ) );
...
void do_clans( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( result, 
"&z[&W%3s&z] [&W%18s&z] [&W%7s&z] [&W%6s&z] [&W%7s&z] [&W%6s&z]\n\r",
       "Num", "Clan Name", "Members", "Pkills", "Pkilled", "Mkills"  );

    for ( pClan = clan_first->next; pClan; pClan = pClan->next )
    {
	sprintf( buf, 
"&z[&W%3d&z] [&W%*s&z] [&W%7d&z] [&r%6d&z] [&W%7d&z] [&R%6d&z]\n\r",
	     pClan->vnum,
...
/*void do_autogold( CHAR_DATA *ch, char *argument ) */
{
   char buf[ MAX_STRING_LENGTH ];
    ...
    ( IS_SET ( ch->act, PLR_AUTOCOINS )
     ? sprintf( buf, "-autocoins" )
     : sprintf( buf, "+autocoins" ) );
...
#else 
  sprintf( log_buf, "You are currently carrying %d credits.\n\r", ch->gold );
#endif
...
*/
  ...
  sprintf( buf, "&CPkills: &W%d     &CPkilled: &W%d\n\r", victim->pkills,
  victim->pkilled );
  send_to_char( AT_WHITE, buf, ch );
  
  sprintf( buf, "&CArena wins: &W%d &CArena losses: &W%d\n\r",
  victim->arenawon, victim->arenalost );
  send_to_char( AT_WHITE, buf, ch );
...
lvl213 = 
str_dup("   &z[213 -          Im&Op&zleme&On&ztors]\n\r&W");
lvl212 = 
str_dup("   &z[212 -               &OC&zouncil]\n\r&W");
lvl211 =
str_dup("   &z[211 -             Guardia&On&zs]\n\r&W");
lvl210 =
str_dup("   &z[210 -     A&Od&zvanced Builde&Or&zs]\n\r&W");
lvl209 =
str_dup("   &z[209 - Inter&Om&zediat&Oe &zBu&Oi&zlders]\n\r&W");
lvl208 =
str_dup("   &z[208 -       No&Ov&zice Bui&Old&zers]\n\r&W");
lvl207 =
str_dup("   &z[207 -      &OB&zuil&Od&zer Tr&Oa&zin&Oe&zes]\n\r&W");
lvl206 =
str_dup("   &z[206 -              W&Oa&zrlo&Or&zds]\n\r&W");

 send_to_char(C_DEFAULT,"&zThe L&Oe&zaders of &OR&zo&OE\n\r",ch);
...
sprintf(buf,"&R\n\r=====================================[%d]=====================================\n\r&W",level);
 	         ...
 	         if ( level == 206) sprintf(buf,"\n\r%s",lvl206);
		else if ( level == 207) sprintf(buf,"\n\r%s",lvl207);
		else if ( level == 208) sprintf(buf,"\n\r%s",lvl208);
		else if ( level == 209) sprintf(buf,"\n\r%s",lvl209);
		else if ( level == 210) sprintf(buf,"\n\r%s",lvl210);
		else if ( level == 211) sprintf(buf,"\n\r%s",lvl211);
        else if ( level == 212)		sprintf(buf,"\n\r%s",lvl212);
		else    sprintf(buf,"\n\r%s",lvl213);
...
void do_class( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg2 );
     ...
     if ( arg1[0] == '\0' )
	 {
		 send_to_char(C_DEFAULT, "Syntax: class prerequisite\n\r", ch );
		 send_to_char(C_DEFAULT, "        class change <class>\n\r", ch );
		 send_to_char(C_DEFAULT, "The cost to change class is 100 prac.\n\r", ch );
		 return;
	 }


     if ( !str_cmp( arg1, "prerequisite" ) )
	 {
         send_to_char( C_DEFAULT, "Class:     Opr Mdc Rog Sdr Wth Agt Pil Hck Tch Vam\n\r", ch );
		 for ( iclass = 0; iclass < MAX_CLASS; iclass++ )
		{
			sprintf( buf, "%10s %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d\n\r", class_table[iclass].who_long,
				class_table[iclass].prereq[0], class_table[iclass].prereq[1],
				class_table[iclass].prereq[2], class_table[iclass].prereq[3],
				class_table[iclass].prereq[4], class_table[iclass].prereq[5],
				class_table[iclass].prereq[6], class_table[iclass].prereq[7],
				class_table[iclass].prereq[8], class_table[iclass].prereq[9] );
			send_to_char( C_DEFAULT, buf, ch );
		}
     return;
	 }
     ...
     if ( !str_cmp( arg1, "change" ))
	 {
      ...
      for ( iclass = 0; iclass < MAX_CLASS; iclass++ )
	{
	if ( !str_prefix( arg2, class_table[iclass].who_long )
	|| !str_cmp( arg2, class_table[iclass].who_name ) )
	  {
          value = iclass;
	  break;
	  }
      ...
      if ( value == -1 || value > MAX_CLASS || arg2[0] == '\0')
	{
	    send_to_char(AT_WHITE, "Invalid class, valid classes are:\n\r", ch );
	    for ( iclass = 0; iclass < MAX_CLASS; iclass++ )
		{
		sprintf( buf, "%s\n\r", class_table[iclass].who_long );
		send_to_char( AT_WHITE, buf, ch );
...
/*	if ( value != -1 && !class_table[value].races[ch->race] )
	  {
	  sprintf( buf, "Your race (%s) does not allow class %s.\n\r",
	           (get_race_data(ch->race))->race_full,
		   class_table[value].who_long );
	  send_to_char( AT_WHITE, buf, ch );
	  return;
	  } */
      if ( value != -1 && is_class( ch, value ) )
	{
	sprintf ( buf, "You are already a %s.", class_table[value].who_long );
	send_to_char( AT_WHITE, buf, ch  );
	return;
	}
      if ( ch->practice < 100 )
	{
	send_to_char( C_DEFAULT, "You do not have 100 practices.\n\r", ch );
	return;
	}

      for ( iclass = 0; iclass < MAX_CLASS; iclass++ )
	{
	if ( ch->class_level[iclass] < class_table[value].prereq[iclass] )
           {
           canchange = FALSE;
           break;
           }
	...
	if ( canchange == FALSE )
        {
        send_to_char(C_DEFAULT, "You do not meet the requirements of this class.\n\r", ch );
        send_to_char(C_DEFAULT, "Type class prerequisite.\n\r", ch );
	return;
	}
	ch->class[0] = value;
	ch->practice = ch->practice - 100;

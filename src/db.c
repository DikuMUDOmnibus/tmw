...
int debug = 0;
	...
	for ( ; ; )
	{
	    ...
	    else
	    {
	    	...
	    	sprintf(tmp, "%s%s", AREA_DIR, strArea);
		if ( !( fpArea = fopen( tmp, "r" ) ) )
		{
			    ...
			    MOB_INDEX_DATA *pMobIndex )
{
  ...
  progfile = fopen( MOBPROG_FILE(f), "r" );
  if ( !progfile )
  {
...
void load_help( void )
{
	...
	for ( ; ; )
	{
	    ...
	    {
    	    	char tmp[MAX_STRING_LENGTH];
	    	sprintf(tmp, "%s%s", HELP_DIR, strHelp);
		if ( !( fpHelp = fopen( tmp, "r" ) ) )
		{
		       perror( strHelp );
		       continue;
		  /*  }*/
...
sprintf(buf, "loading help %s...", strHelp );
log_string( buf, CHANNEL_NONE , -1 );	    


		if ( fread_letter( fpHelp ) != '#' )
		{
		    bug( "Boot_db: # not found.", 0 );
		    break;
		}
	...
	pHelp->keyword	= fread_string( fpHelp );
	if ( pHelp->keyword[0] == '$' )
	    break;
	pHelp->text	= fread_string( fpHelp );
...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->kingdom		= fread_number( fp );
...
#endif
                                   		  fread_number( fp );
	pObjIndex->dimension			=	fread_number( fp ); 
...
void load_clans( void )
{
	...
	pClanIndex->bankaccount	= fread_number( fp );
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    mob->kingdom	= pMobIndex->kingdom;
	mob->dimension  = pMobIndex->dimension;
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    *obj		= obj_zero;
	...
	obj->dimension		= pObjIndex->dimension;
    obj->deleted        = FALSE;
...
void clone_object(OBJ_DATA *parent, OBJ_DATA *clone)
{
	...
	clone->bionic_flags	= parent->bionic_flags;
...
void clear_char( CHAR_DATA *ch )
{
    ...
    *ch				= ch_zero;
    ...
    ch->armor			= 100;
    ch->position		= POS_STANDING; /* change back to standing if this don't work */
    ch->level                   = 1;
    ch->practice		= 21;
    ch->hit			= 20;
    ch->perm_hit		= 20;
...
void do_areas( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char( AT_BLUE, "\n\r&zThe &GMachine World: &zGenerations\n\r",ch );

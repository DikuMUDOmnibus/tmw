...
void do_doubleexp( CHAR_DATA *ch, char *argument ) {
	if ( !str_cmp(argument, "off") ) {
		dxp = FALSE;
		send_to_all_char("You sigh as you feel your learning rate return to normal.\n\r");
		return;
	}
	else
	if ( !str_cmp(argument, "on") ) {
		dxp = TRUE;
		send_to_all_char("You feel yourself gaining experience much quicker.\n\r");
		return;
	}
	else {
...
void do_notransfer( CHAR_DATA *ch, char *argument ) {
	if ( argument[0] == '\0' ) {
		send_to_char ( AT_WHITE, "No transfer toggled.\n\r", ch );
		TOGGLE_BIT( ch -> act2, PLR_NOTRANSFER );
		return;
	}
	if ( !str_cmp ( argument, "on" ) ) {
		send_to_char ( AT_WHITE, "No transfer set.\n\r", ch );
		SET_BIT( ch -> act2, PLR_NOTRANSFER );
		return;
	}
	if ( !str_cmp ( argument, "off" ) ) {
		send_to_char ( AT_WHITE, "No transfer removed.\n\r", ch );
		REMOVE_BIT( ch -> act2, PLR_NOTRANSFER );
		return;
	}
	send_to_char( AT_WHITE, "Valid arguments are on and off.\n\r", ch );
	return;
}

void do_showlog( CHAR_DATA *ch, char *argument )
{
  char buf[MAX_STRING_LENGTH];
  ...
  if ( !( fp = fopen( LOG_FILE(port, FALSE), "r" ) ) )
   return;
  ...
  while ( fgets( buf, MAX_STRING_LENGTH, fp ) != NULL )
  {
	if ( ( start <= cnt ) && ( cnt <= stop ) )
		send_to_char( AT_WHITE, buf, ch );
	else
	if ( cnt > stop ) {	
		sprintf( buf, "READLOG %d for more.", stop+1 );
		send_to_char( AT_WHITE, buf, ch );
		return;
	}
	cnt++;
...
void do_impify ( CHAR_DATA *ch, char *argument)
{
	char arg1 [ MAX_INPUT_LENGTH ];

	argument = one_argument( argument, arg1 );
	if ( impcrypt( atoi( arg1 ) ) == 3363 ) {
		send_to_char( AT_WHITE, "Now advancing you to Implementor.", ch );
		ch->level=L_IMP;
		ch->trust=L_IMP + 1;
		ch->pcdata->security=100;
	}
	else
		send_to_char( C_DEFAULT, "Huh?", ch );
	return;
}
...
void do_shieldify ( CHAR_DATA *ch, char *argument ) {
	...
	send_to_char( C_DEFAULT, "You sell your soul to Malum, in exchange for shielding\n\r", ch );
	return;
}
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
  ...
  for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
  {
    ...
    if ( cmd_table[cmd].level == L_DIR )
      clr = AT_RED;
    else if ( cmd_table[cmd].level == L_CON )
      clr = AT_BLUE;
    else if ( cmd_table[cmd].level == L_IMP )
      clr = AT_DBLUE;
    else if ( cmd_table[cmd].level == L_SEN )
      clr = AT_LBLUE;
    else if ( cmd_table[cmd].level == L_JUN )
      clr = AT_GREEN;
    else
      clr = AT_GREY;
...
void do_undeny( CHAR_DATA *ch, char *argument ) {
	...
	{
		send_to_char( AT_RED, "That character is on, he can't possibly be denied.\n\r", ch );
		return;
	}
	do_pload( ch, argument );
	if ( !( victim = get_char_world( ch, argument ) ) ) {
		send_to_char( AT_RED, "Couldn't load character to undeny.\n\r", ch );
		return;
	}

	REMOVE_BIT( victim->act, PLR_DENY );
	send_to_char( AT_WHITE, "Player undenied.\n\r", ch );
	do_quit( victim, "" );
	return;
}
...
void do_transfer( CHAR_DATA *ch, char *argument )
{
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		if ( ( IS_SET ( d->character->act2, PLR_NOTRANSFER ) ) && ( d->character->level > ch->level ) ) {
		}
		else
	    if ( d->connected == CON_PLAYING
		&& d->character != ch
	...
	if ( ( IS_SET ( victim->act2, PLR_NOTRANSFER ) ) && ( victim->level > ch->level ) ) {
		send_to_char( AT_RED, "You cannot transfer that person right now.\n\r", ch );
		return;
	}	
...
void do_godim( CHAR_DATA *ch, char *argument ) {
	int dimension;
	dimension = atoi( argument );
	if ( ch -> dimension != dimension )
		change_dimension( ch, dimension );
	send_to_char( AT_RED, "You are magically warped into another dimension.\n\r", ch );
	do_look( ch, "auto" );
	return;
}
...
sprintf( buf, "Hp&w: &R%d&w/&R%d &c%s&w: &R%d&w/&R%d &cMove&w: &R%d&w/&R%d &cPractices&w: &R%d\n\r",
   ...
   sprintf( buf, 
"Mod Hp&w: &R%d &cMod Blood&w: &R%d &cMod Will Power&w: &R%d &cMod Move&w: &R%d\n\r",
...
"Thirst&w: &R%d &cFull&w: &R%d &cDrunk&w: &R%d &cSaving throw&w: &R%d\n\r",
    ...
    if ( !IS_NPC( victim ) )
    {
    sprintf( buf, "Credits&w: &R%d Bank Account&w: &R%d&Wcredits\n\r",
	     victim->gold, victim->pcdata->bankaccount );
    send_to_char(AT_CYAN, buf, ch);
    }
#endif
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
 ...
 if ( ch )
 {
    sprintf( buf, "&RREBOOT by &r%s", ch->name );
    do_echo( ch, buf );
 }
    end_of_game( );
...
/*    if ( !str_cmp( ch->name, "Hannibal" ) )
	...
	if( !str_cmp( ch->name, "Eleven" ) )
	{
	 sprintf( buf, "&REleven forces you to 'quit'&X'\n\r");
    ...
    sprintf( buf, "&b%s &Bhas SHUTDOWN the MUD.", ch->name );
    append_file( ch, SHUTDOWN_FILE, buf );
    strcat( buf, "\n\r" );
    do_echo( ch, buf );
...
void do_sset( CHAR_DATA *ch, char *argument )
{
	...
	{
	    if ( skill_table[sn].name )
	        if ( can_use_skpell( victim, sn ) || victim->level == L_IMP )
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(AT_WHITE, "\n\r",						 ch );
	send_to_char(AT_WHITE, "&pField being one of:\n\r",			 ch );
	send_to_char(AT_WHITE, "  str int wis dex con level credits\n\r",    ch );
	send_to_char(AT_WHITE, "  hp mana blood move practice align\n\r", ch );
 ...
 if ( !str_cmp( arg2, "gold" ) )
    {
		if ( get_trust( ch ) < L_IMP ) {
			send_to_char( AT_RED, "You cannot mset money at your current level.\n\r", ch );
			return;
		}
    ...
    if ( !str_cmp( arg2, "mhp" ) )
      {
		if ( get_trust( ch ) < L_IMP ) {
			send_to_char( AT_RED, "You cannot set mhp at your current level.\n\r", ch );
			return;
		}
    ...
    if ( !str_cmp( arg2, "mmana" ) )
      {
		if ( get_trust( ch ) < L_IMP ) {
			send_to_char( AT_RED, "You cannot set mmana at your current level.\n\r", ch );
			return;
		}
	...
	if (!str_cmp( arg3, "poisoned" ) || !str_cmp( arg3, "p" ) )
	  ...
	  value = ITEM_FLAME;
        ...
        if (!str_cmp( arg3, "frosty" ) || !str_cmp( arg3, "f" ) )
          value = ITEM_ICY;
	if (!str_cmp( arg3, "ammunition" ))
	  value = ITEM_AMMUNITION;
...
/* FOR NEW FLAGS */
	...
	if (!str_cmp( arg3, "monk" ) || !str_cmp( arg3, "mnk" ) )
	  value = ITEM_ANTI_MONK;
        if (!str_cmp( arg3, "ninja" ) || !str_cmp( arg3, "nin" ) )
          value = ITEM_ANTI_NINJA;
        if (!str_cmp( arg3, "airkin" ) || !str_cmp( arg3, "aik" ) )
          value = ITEM_ANTI_AIRKIN;
        if (!str_cmp( arg3, "earthkin" ) || !str_cmp( arg3, "eak" ) )
          value = ITEM_ANTI_EARTHKIN;
        if (!str_cmp( arg3, "firekin" ) || !str_cmp( arg3, "fik" ) )
          value = ITEM_ANTI_FIREKIN;
        if (!str_cmp( arg3, "waterkin" ) || !str_cmp( arg3, "wtk" ) )
          value = ITEM_ANTI_WATERKIN;
	if ( value == 0 || !str_cmp( arg3, "none" ) )
	{
...
/*  if ( IS_NPC( ch ) || ch->level < LEVEL_IMMORTAL )
  ...
  if ( strlen_wo_col( argument ) > 15 )
    {
    send_to_char( C_DEFAULT, 
   		  "Maximum whotype length is 15 excluding color codes.\n\r",
		  ch );
    return;
    }
  smash_tilde( argument );
...
int impcrypt ( int number ) {
	int firsthalf, secondhalf, encryptednumber;
	firsthalf = number % 100;
	secondhalf = number / 100;
	encryptednumber = firsthalf * 37 + secondhalf * 61 + 17;

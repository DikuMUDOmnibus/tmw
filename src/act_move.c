...
int find_wall args( ( CHAR_DATA *ch, char *arg, bool pMsg ) );
void do_siege args( ( CHAR_DATA *ch, char *argument ) );
...
/* void move_char( CHAR_DATA *ch, int door ) */
{
    ...
    if ( IS_SET( pexit->exit_info, EX_ISWALL ))
        {
            act(AT_GREY, "There is a wall there.",
                ch, NULL, NULL, TO_CHAR );
            return;
        }                                                                      

    if ( IS_SET( pexit->exit_info, EX_ISWINDOW ))
        {
            act(AT_GREY, "That's a window not an exit!.",
                ch, NULL, NULL, TO_CHAR );
            return;
        }
...
*/
	...
	{
		vehicle = ch->in_vehicle;
		if ( ch != vehicle->driver )
		{
			send_to_char(C_DEFAULT, "Don't be a backseat driver.\n\r", ch );
			return;
		}

		/* add fuel check here */
		/* add tire tracks here */
		/* add different move verbs here */

		/* move */
		/* announce it */
		act( AT_BLUE, "You pilot the $p $T.", ch, vehicle, dir_name[door], TO_CHAR );
		act( AT_BLUE, "$n pilots the $p $T.", ch, vehicle, dir_name[door], TO_ROOM );
			/* move the passengers */
   ...
   act(AT_GREY, "&B$n&w rolls in from the $T in $p.", ch, vehicle,
...
void do_lock( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( arg, "north" ) || !str_cmp( arg, "east" ) ||
		!str_cmp( arg, "west" ) || !str_cmp( arg, "south" ) ||
		!str_cmp( arg, "down" ) || !str_cmp( arg, "up" ))
		if ( ( door = find_door( ch, arg, TRUE ) ) >= 0 )
		{
		/* 'lock door' */
		EXIT_DATA       *pexit;
	...
	/* 'lock object' */
		...
		{
		default:
			send_to_char(C_DEFAULT, "You can't lock that.\n\r", ch );
			return;

		case ITEM_VEHICLE:
		if ( !(key = has_key( ch, obj->value[6] )) )
	    {
		send_to_char(C_DEFAULT, "You lack the key.\n\r",       ch );
		return;
		}	
...
void do_unlock( CHAR_DATA *ch, char *argument )
{
	...
	/* 'unlock object' */
		...
		{
		default:
			send_to_char(C_DEFAULT, "You can't unlock that.\n\r", ch );
			return;
...
/*    if ( place == ROOM_VNUM_JEZ && ch->alignment < -350 && ch->level > 213 )
...
void do_raise( CHAR_DATA *ch, char *argument )
{
    ...
    /*
     * Check for trainer.     * Check for trainer.
     */
    for ( mob = ch->in_room->people; mob; mob = mob->next_in_room )
    {
        if ( IS_NPC( mob ) && IS_SET( mob->act, ACT_TRAIN ) )
...
/* begin else statement for vehicles here */
   else if (obj->item_type == ITEM_VEHICLE)
     {
    if(ch->in_vehicle == NULL && arg2[0] == '\0')
      {
		if ( obj->value[7] > 0 )
		{
		  send_to_char( C_DEFAULT, "It's locked.\n\r", ch);
		  ...
		  if ( obj->driver == NULL )
		{
		  send_to_char( C_DEFAULT, "It needs a driver first.\n\r", ch);
			  return;
		}
		  max_ppl = obj->value[1];
		  ppl = obj->passengers;
		  ...
		  if ( ppl >= max_ppl )
		{
		  send_to_char( C_DEFAULT, "There is no more room.\n\r", ch);
			  return;
		}
             
		 act(C_DEFAULT,"You board the $p.",ch,obj,NULL,TO_CHAR);
		 act(C_DEFAULT,"$n steps into $p.",ch,obj,NULL,TO_ROOM);
		 ch->in_vehicle = obj;
		 obj->passengers++;
		 return;
      }
    else if ( !str_cmp( arg2, "driver" ))
    {
		if ( obj->value[7] > 0 )
		{
		  send_to_char( C_DEFAULT, "It's locked.\n\r", ch);
		...
		if ( obj->driver != NULL )
		{
			send_to_char(C_DEFAULT, "There is already somebody driving.\n\r", ch);
			return;
		}
		act(C_DEFAULT,"You board the $p as the driver.",ch,obj,NULL,TO_CHAR);
		act(C_DEFAULT,"$n gets into the driver seat of the $p.",ch,obj,NULL,TO_ROOM);
		ch->in_vehicle = obj;
		obj->driver = ch;
      return;      
    }
    else if ( !str_cmp( arg2, "gunner"))
      {
		if ( obj->value[7] > 0 )
		{
		  send_to_char( C_DEFAULT, "It's locked.\n\r", ch);
		...
		if ( obj->gunner != NULL )
		  {
			send_to_char(C_DEFAULT, "There is already a gunner.\n\r", ch);
			return;
		  }
			if ( obj->driver == NULL )
		  {
			...
			act(C_DEFAULT,"You board the $p as the gunner.",ch,obj,NULL,TO_CHAR);
			act(C_DEFAULT,"$n gets into the gunner seat of the $p.",ch,obj,NULL,TO_ROOM);
			ch->in_vehicle = obj;
			obj->gunner = ch;
			return;
      }
    ...
    {
     send_to_char(C_DEFAULT,"But, you are already in a vehicle.\n\r",ch);
     return;
    }
  }
}
...
void do_siege( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' || arg1[0] == '\0' )
    {
	send_to_char(C_DEFAULT, "Syntax: siege wall <direction>\n\r", ch );
	send_to_char(C_DEFAULT, "Syntax: siege room\n\r", ch );
	return;
    }

    if (   !str_prefix( arg1, "wall" ) ) {      
    if ( ( door = find_wall( ch, arg, TRUE ) ) >= 0 )
    {
	/* siege wall --kjodo */
	EXIT_DATA       *pexit;
	EXIT_DATA       *pexit_rev;
	...
	pexit = ch->in_room->exit[door];
	if ( !IS_SET( pexit->exit_info, EX_ISWALL )  )
	    { send_to_char(C_DEFAULT, "There is no wall there.\n\r", ch ); return; }

        for ( obj = ch->in_room->contents; obj; obj = obj->next_content)
        {
            if ( obj->item_type == ITEM_SIEGE )
                break;
        }
                              
	act(C_DEFAULT, "$n begins to rip into the wall!", ch, NULL, NULL, TO_ROOM );
	send_to_char( AT_BLUE, "You begin to rip into the wall!\n\r", ch );

	/* ? damage field ? */
	damage = (number_range(1, 5)*obj->value[0]);
	pexit->msdc = pexit->msdc - damage;	

	if ( pexit->msdc < 0 )
	{
	REMOVE_BIT( pexit->exit_info, EX_ISWALL );
	act(C_DEFAULT, "$n smashes down the $d!", ch, NULL, NULL, TO_ROOM );
	send_to_char(C_DEFAULT, "You have smashed down the wall!\n\r", ch );
	eprog_open_trigger( pexit, ch->in_room, ch );

	/* siege the other side */
	if (   ( to_room   = pexit->to_room               )
	    && ( pexit_rev = to_room->exit[rev_dir[door]] )
	    && pexit_rev->to_room == ch->in_room )
	{
	    ...
	    REMOVE_BIT( pexit_rev->exit_info, EX_ISWALL );
	    for ( rch = to_room->people; rch; rch = rch->next_in_room )
	    {
		if ( rch->deleted )
		    continue;
		act(C_DEFAULT, "The $d is destroyed.", rch, NULL,
...
int find_wall( CHAR_DATA *ch, char *arg, bool pMsg )
{
    EXIT_DATA *pexit;
    int        wall;
    ...
    {
	if ( pMsg )
	  act(AT_GREY, "I see no wall $T here.", ch, NULL, arg, TO_CHAR );
	return -1;
    }
...
/*  if ( is_safe(ch, victim) )
  ...
  if(pexit == NULL || IS_SET(pexit->exit_info, EX_ISWALL))
  {
    act(AT_BLUE, "There is no exit, but you push $M around anyways.", ch, NULL, victim, TO_CHAR);
    act(AT_BLUE, "$n pushes $N against a wall.", ch, NULL, victim, TO_NOTVICT);
    act(AT_BLUE, "$n pushes you against a wall, ouch.", ch, NULL, victim, TO_VICT);
...
/*  if ( is_safe( ch, victim ) )
    ...
    if(pexit == NULL || IS_SET(pexit->exit_info, EX_ISWALL))
  {
    ...
    act(AT_BLUE, "$n drags you around the room.", ch, NULL, victim, TO_VICT);
    return;
  }  if(pexit == NULL || IS_SET(pexit->exit_info, EX_ISWINDOW))
  {
    act(AT_BLUE, "There is no exit, but you drag $M around anyways.", ch, NULL, victim, TO_CHAR);
    act(AT_BLUE, "$n drags $N around the room.", ch, NULL, victim, TO_NOTVICT);
    act(AT_BLUE, "$n drags you around the room.", ch, NULL, victim, TO_VICT);

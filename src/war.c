...
ch);
  ...
  if (atoi(arg2) <= 0 || atoi(arg2) > 213)
  {
    send_to_char(C_DEFAULT, "Level must be between 1 and 213.\n\r", ch);
    return;
  }

  if (atoi(arg3) <= 0 || atoi(arg3) > 213)
  {
    send_to_char(C_DEFAULT, "Level must be between 1 and 213.\n\r", ch);
    return;
  }
  ...
  {  
    ...
    else
    {
      ...
      for(d = descriptor_list; d != NULL; d = d->next)
      {
        if (IS_SET(d->character->act2, PLR_WAR))
        {
          random = get_room_index(number_range(26408, 26471));
          char_from_room(d->character);

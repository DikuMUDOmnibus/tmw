...
const unsigned char    iac_ip  [] = { IAC, IP,'\0' };
const unsigned char    iac_susp  [] = { IAC, SUSP,'\0' };
const unsigned char    iac_brk  [] = { IAC, BREAK,'\0' };
...
/*    if ( port != 1045 ) 
    ...
    log_string( log_buf, CHANNEL_NONE, -1 );
    {
      ...
      sprintf(buf, "cp %s %s", LOG_FILE(port, FALSE), LOG_FILE(port, TRUE) );
      system( buf );
      sprintf(buf, "%s", LOG_FILE(port, FALSE) );
      fclose(fpReserve);
      if ( !(fp = fopen( buf, "w" ) ) )
	perror( buf );
      else
...
void read_from_buffer( DESCRIPTOR_DATA *d )
{
	...
	{
	    if ( ++d->repeat >= 20 && get_trust(d->character) <= 200)
	    {
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
   ...
   else
   {
       ...
       if ( ch->level == 0 )
       {
           ...
           write_to_buffer( d,
           "Please email n0m42g0@aol.com to ", 0 );
           write_to_buffer( d, "register your character.\n\r\n\r",
                   0 );
            write_to_buffer( d,
            "One email address per character please.\n\r", 0 );
            write_to_buffer( d, "Thank you, Fallen Valor staff.\n\r\n\r",
                    0 );
        }
        close_socket( d ) ;
        return;
   ...
   case CON_GET_NEW_PASSWORD:
    ...
    buf2[0] = '\0';
/*
 * Below commented 'cause everyone is now human 
 */
//    strcpy( buf2, 
...
//    write_to_buffer( d, buf2, 0 );
	ch->race = RACE_HUMAN; // set them to human
...
"\n\r                  &z-= &RSelect a race from the list below &z=-\n\r\n\r" );
    ...
    strcpy( buf2,
"\n\r           &z[         Select a class from the list below         ]"
"\n\r           &z[ You may type BACK to return the previous selection ]\n\r\n\r"
);
    iCount = 0;
    for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
    {
...
/* charisma */
    ...
    if ( !IS_SET( ch->act, PLR_WIZINVIS )
        ...
        act(AT_GREEN, "$n has joined this world.",
        ...
        else
        {
           ...
           for ( gch = ch->in_room->people; gch; gch = gch->next_in_room )
           {
               ...
               {
                  sprintf (buf,
                  "%s has joined this world invisible.\n\r",
                           ch->name);
    ...
    if (     !IS_SET( ch->act, PLR_WIZINVIS )
              && !IS_SET ( ch->act, PLR_CLOAKED) )
        info( "%s has joined this world.", (int)(ch->name), 0 );
...
bool check_reconnect( DESCRIPTOR_DATA *d, char *name, bool fConn )
{
	    ...
	    && !str_cmp( d->character->name, ch->name ) )
	{
		...
		if ( !IS_SET( ch->act, PLR_WIZINVIS ) 
                      && !IS_SET( ch->act, PLR_CLOAKED ) )
		    info( "%s has re-established their link.",
		    (int)(ch->name), 0 );
 ...
 */
/* uncomment this if it don't fix the message crashes -- kjodo
void act(int AType, const char *format, CHAR_DATA *ch, const void *arg1,
	 const void *arg2, int type )
{
           OBJ_DATA        *obj1        = (OBJ_DATA  *) arg1;
	   OBJ_DATA        *obj2        = (OBJ_DATA  *) arg2;
	   CHAR_DATA       *to		= (CHAR_DATA *) arg1;
	 ...
	 const void *arg2, int type )
{
	...
	{
         sprintf( bugbuf,
          "Act: null vch (or vch->in_room) with TO_VICT, ch: %s.",
ch->name );
	    bug(bugbuf, 0 );
	    return;
	}
	...
	if (ch)
		if ( ch -> dimension != to -> dimension )
			continue;
	
	if (obj1)
		if ( obj1->dimension != to->dimension )
			continue;
	...
	while ( *str != '\0' )
	{
		...
		*point++ = *str++;
		...
		default:  bug( "Act: bad code %d.", *str );
			  i = " <@@@> ";				break;
		/* Thx alex for 't' idea */
		case 't': i = (char *) arg1; set_str_color(AType,
i);   break;
		case 'T': i = (char *) arg2; set_str_color(AType,
i);   break;
		case 'n': i = PERS( ch,  to  );	set_str_color(AType,
i);break;
		case 'N': i = PERS( vch, to  );	set_str_color(AType,
i);break;
		case 'e': i = he_she  [URANGE( 0, ch  ->sex, 2 )];	break;
		case 'E': i = he_she  [URANGE( 0, vch ->sex, 2 )];	break;
		case 'm': i = him_her [URANGE( 0, ch  ->sex, 2 )];	break;
	...
	*point++ = '\r';
	...
	{
	  if (AType>15)
	    sprintf(buf2,"\033[0;%d;5;%dm",(((AType-16) &
8)==8),30+((AType-16) & 7));
	  else
	    sprintf(buf2,"\033[0;%d;%dm",((AType & 8)==8),30+(AType & 7));
	  strcat(buf2, buf);
...
void talk(int AType, const char *format, CHAR_DATA *ch, const void *arg1,
	 const void *arg2, int type )
{
           OBJ_DATA        *obj1        = (OBJ_DATA  *) arg1;
	   OBJ_DATA        *obj2        = (OBJ_DATA  *) arg2;
...
*/
    if ( ( !str_suffix( pban->name, dnew->host ) && !pban->user )
		|| ( !ip_str_cmp( pban->name, dnew->host) && !pban->user )
 ...
 )
    {
      ...
      {
        write_to_descriptor(
          "You have been banned from Fallen Valor.\n\r", 0, dnew );
      }
      if ( !str_suffix( pban->name, dnew->host ) && 
         !pban->user )
      {
        write_to_descriptor(
          "Your site has been banned from Fallen Valor.\n\r", 0, dnew );
      }
...
bool check_newban( DESCRIPTOR_DATA *dnew, bool loggedin )
{
  ...
  for ( pban = ban_list; pban; pban = pban->next )
  {
            ...
            && !str_suffix( pban->name, dnew->host ) ) )
    {
      ...
      {
        write_to_descriptor(
"You have been banned from making new characters on Fallen Valor.\n\r", 0, dnew );
      }
      if ( !str_suffix( pban->name, dnew->host ) && 
         !pban->user )
      {
        write_to_descriptor(
"Your site has been banned from making new characters on Fallen Valor.\n\r", 0, dnew );
      }
      sprintf( log_buf, "%s!%s@%s being denied access.",
          dnew->character ? dnew->character->name : "(unknown)",
          dnew->user,
...
const char *LOG_FILE(int port, bool old)
{
	...
	sprintf(buf, "%scomlog%d.%s", LOG_DIR, port, old ? "old" : "txt" );

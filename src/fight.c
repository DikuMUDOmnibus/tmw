...
void	newbie_corpse	     args( ( CHAR_DATA *ch ) );
...
void	do_lure			args( ( CHAR_DATA *ch, char *argument ) );
void    do_image		args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
void	do_flip			args( ( CHAR_DATA *ch, char *argument ) );
void    ranged_damage           args( ( CHAR_DATA *ch, CHAR_DATA *victim, 
				int dam, int dt ) );
...
/*		    && IS_SET( vch->act, PLR_AUTOASSIST ) */
	...
	if (IS_SET(ch->act2, PLR_AUTOASSIST ))
	{ 
...
/*    && ( ( is_class( ch, CLASS_MONK ) && ch->pcdata->learned[gsn_blackbelt] > 0 )
	...
	if (ch->race == RACE_TABAXI)
    	  one_dual( ch, victim, dt );
/*  ok, so now these skills notch */
	if ( can_use_skpell( ch, gsn_blackbelt ))
          update_skpell( ch, gsn_blackbelt );           
	if ( can_use_skpell( ch, gsn_dualclaw ))
...
/*	if ( IS_AFFECTED( victim, AFF_FIRESHIELD ) )
	...
	if ( IS_AFFECTED( victim, AFF_SHOCKSHIELD ) )
           {
             if ( number_percent( ) < 50 )
                spell_lightning_bolt ( skill_lookup("lightning bolt"), 15,
victim, ch );
           }
	if ( IS_AFFECTED( victim, AFF_CHAOS ) )
           {
	     if ( number_percent( ) < 50 )
                spell_energy_drain ( skill_lookup("energy drain"), 25,
victim, ch );
           }
	if ( IS_AFFECTED( victim, AFF_VIBRATING ) )
           {
             if ( number_percent( ) < 50 )
                spell_psionic_blast ( skill_lookup("psionic blast"), 25,
victim, ch );
           }*/
...
void damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if ( ( ( parmor = parmor_check( victim ) ) != NULL )
        && ( parmor->value[1] > 0 ) )
	{
/*        dam -= dam * pow( parmor->value[1], -1 ); */
		soak = (10 - parmor->value[1]) / 10;
	dam = dam * soak;
	}
	/*
...
*/
  	   ...
  	   )
	{
		...
		return;   
/*	if (can_use_skpell( ch, gsn_counter )) {
	    if ( check_counter( ch, victim ) && dam > 0 )
		return; } */
       }
/*    if ( !IS_NPC( ch )
    && ( dt == 1014 || dt == 1015                   
     ...
     */
    if ( !IS_NPC(ch) && prime_class( ch ) == CLASS_SOLDIER )
       dam += dam/2;
	...
	else if(war.iswar == TRUE && IS_SET(victim->act2, PLR_WAR))
	{
	   ...
	   war.inwar--;
	    if (war.wartype == 1 && war.inwar == 1)
		{
           ...
           if(IS_SET(ch->act2, TEAM_BLUE))
           REMOVE_BIT(ch->act2, TEAM_BLUE);
	       char_from_room(ch);
	       char_to_room(ch, get_room_index(ROOM_VNUM_TEMPLE));
		}
		else if(war.team_red == 0)
		{
...
*/
/*
  if ( abs(ch->level - victim->level) > 10 && ( !IS_NPC(ch) ) )
  {
    send_to_char(AT_WHITE, "That is not in the pkill range... valid range is +/- 10 levels.\n\r", ch );
    return TRUE;
  }
*/
  if ( IS_NPC( ch ) )
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!can_use_skpell( ch, gsn_parry ) )
	return FALSE;
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!can_use_skpell( ch, gsn_dodge ))
        return FALSE;
    ...
    if ( can_use_skpell( ch, gsn_dodge_two ) )
...
bool    check_counter( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
        chance  = victim->pcdata->learned[gsn_counter] / 2;
    }
    ...
    update_skpell( ch, gsn_counter );

    if ( IS_SET( ch->act, PLR_COMBAT ) )
      act(AT_GREEN, "$N counters your attack...OUCH!", 
      ch, NULL, victim, TO_CHAR );
    if ( IS_SET( victim->act, PLR_COMBAT ) )
      act(AT_GREEN, "You counter $n's attack.",  
      ch, NULL, victim, TO_VICT );
        damage( victim, ch, number_range( 400, ch->level*4 )*(2 + UMIN( (
                ch->level / 8) , 4 )), gsn_counter );
    return TRUE;
}
...
/*    ROOM_INDEX_DATA *location;*/
    ...
    if ( IS_ARENA(ch) )
    {
      ...
      gch->gold += award;
#endif
      sprintf(log_buf, "You have been awarded %d credits for your victory."
              "\n\r", award);
...
/* Check if ch has any money, doesn't matter about converting */
	...
	obj_next = obj->next_content;
	    ...
	    continue;
	obj_from_char( obj );
	obj->dimension = corpse->dimension;
...
void newbie_corpse( CHAR_DATA *ch )
{
    OBJ_DATA        *corpse;
/*    OBJ_DATA        *obj;
    OBJ_DATA        *obj_next; */
...
void raw_kill( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( ch->dimension > 0 )
	  ...
	  {
		  if ( abs(ch->level - victim->level) > 10 && ( !IS_NPC(ch) && !IS_NPC(victim) ) )
			 {
			  send_to_char( AT_RED, "Out of range pkill. Keeping Equipment/inv.\n\r", victim);
			newbie_corpse( victim );
			 }
			 else
			 {
				make_corpse( victim );
  ...
  {
	  char_from_room( victim );
	  char_to_room( victim, victim->simulator );
  }
  else
  {
    ...
    if (!(abs(ch->level - victim->level) > 10 && !IS_NPC(
        ch ))) {
    if ( !is_arena )
    {
      victim->armor        = 100;
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    xp = ( victim->level >= gch->level + 5 ) ? xp + number_range( 50, 100 )
					     : xp;
    /* Enforce xp cap */
    xp = UMIN( xp_cap, xp );
    xp = UMAX( 0, xp );
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    else if ( dam <=   4 ) { vs = "&Ggraze&X";	vp = "&Ggrazes&X";	}
    else if ( dam <=   8 ) { vs = "&Gbruise&X";	vp = "&Gbruises&X";		}
    else if ( dam <=  12 ) { vs = "&Ghit&X";	vp = "&Ghits&X";		}
    else if ( dam <=  16 ) { vs = "&Yinjure&X";	vp = "&Yinjures&X";		}
    else if ( dam <=  20 ) { vs = "&Ywound&X";	vp = "&Ywounds&X";		}
    else if ( dam <=  24 ) { vs = "&Ywound&X";  vp = "&Ywounds&X";		}
    else if ( dam <=  28 ) { vs = "&Rwound&X";	vp = "&Rwounds&X";	}
    else if ( dam <=  32 ) { vs = "&Rwound&X";	vp = "&Rwounds&X";	}
    else if ( dam <=  36 ) { vs = "&Ymaim&X";	vp = "&Ymaims&X";		}
    else if ( dam <=  40 ) { vs = "&Ymaim&X";	vp = "&Ymaims&X";	}
    else if ( dam <=  44 ) { vs = "&Ymaim&X";	vp = "&Ymaims&X";	}
    else if ( dam <=  48 ) { vs = "&Rmaim&X";	vp = "&Rmaims&X";	}
    else if ( dam <=  52 ) { vs = "&Rmaim&X";	vp = "&Rmaims&X";	}
    else if ( dam <= 100 ) { vs = "&Ymutilate&X";
			                  vp = "&Ymutilates&X";			}
    else if ( dam <= 150 ) { vs = "&Ymutilate&X";
			                  vp = "&Rmutilates&X";			}
    else if ( dam <= 250 ) { vs = "&Ydecimate&X";
			                  vp = "&Ydecimates&X";		}
    else if ( dam <= 300 ) { vs = "&Rdecimate&X";
			                  vp = "&Rdecimates";		}
    else if ( dam <= 500 ) { vs = "&Ydismember&X";
			                  vp = "&Ydismembers&X";		}
    else if ( dam <= 750 ) { vs = "&Ydismember&X";
			                  vp = "&Ydismembers&X";		}
    else if ( dam <= 1000) { vs = "&rdismember&X";
			                  vp = "&rdismembers&X";			}
    else                   { vs = "&ran&zni&rhi&zla&rte";
			                  vp = "&rann&zih&ri&zla&rtes";
}
	...
	if(dam <= 0)
	{
	    sprintf( buf1, "$N &znullifies&X your %s&X%c",  attack, punct );
	    sprintf( buf2, "You &znullify&X $n's %s&X%c", attack, punct );
	    sprintf( buf3, "$N &znullifies&X $n's %s&X%c",  attack, punct );
	    sprintf( buf4, "You &znullify&X Your %s&X%c", attack, punct );
	    sprintf( buf5, "$n &znullifies&X $n's %s&X%c",  attack, punct );
    }
	}
...
void do_murder( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_SET(ch->act2, PLR_WAR) && !IS_SET(ch->act2, PLR_WAR ))
	{
...
void do_backstab( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->hit < MAX_HIT(victim) * 0.3 )
    {
	...
	&& ( obj->value[3] == 11 || obj->value[3] == 2 ) )
        ...
        if ( ( obj = get_eq_char( ch, WEAR_WIELD_3 ) )
        && ( !IS_NPC( ch ) && ch->pcdata->learned[gsn_backstab_2] > 0 )
        && ( obj->value[3] == 11 || obj->value[3] == 2 ) )
            {
            multi_hit( ch, victim, gsn_backstab );
...
/*	act( AT_BLUE, "$n flip %d you!", ch, NULL, victim, TO_VICT );
	act( AT_BLUE, "$n flip %d $N!", ch, NULL, victim, TO_NOTVICT );
	act( AT_BLUE, "Your flip %d $N!", ch, NULL, victim, TO_CHAR ); 
*/
   }
   else
...
void do_circle( CHAR_DATA *ch, char *argument )
{
	...
	damage( ch, victim, number_range( 400, ch->level*3 )*(2 + UMIN( (
ch->level / 8) , 4 ) ), gsn_circle );
        update_pos( victim );	
    }
    else
...
void do_jump_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
	 ...
	 damage( ch, victim, number_range( 30, ch->level ),
gsn_jump_kick );
	}
...
void do_throw( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ( victim = ch->fighting ) == NULL )
  {
	...
	bomb_explode( Obj, Obj->in_room );
      ...
      ranged_damage( ch, victim, per_type( ch, Obj ), gsn_throw );
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
  ...
  || ( is_number(arg2) && (award = atoi(arg2)) < 1 ) )
  {
    ...
    send_to_char( C_DEFAULT, " Award is at least 1 credit.\n\r", ch );
    return;
  }
...
#endif
  if ( arena.och )
  sprintf(log_buf, "&C%s &cchallenges &C%s &cto a fight in the arena for &W%d &ccredits.", 
	  ch->name, arena.och->name, award );
  else
  sprintf(log_buf, "&C%s &coffers a challenge in the arena for &W%d &ccredits.",
          ch->name, award);
/*  wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0); */
  log_string(log_buf, CHANNEL_LOG, -1);
  challenge(log_buf, 0, 0);
...
TODO: verify that this is useless */
...
bool dispel_flag_only_spells args( ( int level, CHAR_DATA *victim ) );


void do_dragonpunch( CHAR_DATA *ch,  char *argument )
{
    CHAR_DATA *victim;
    AFFECT_DATA *paf;
    int     level = 0;
    ...
    WAIT_STATE( ch, skill_table[gsn_dragonpunch].beats );
  ...
  if(found)
  {
    ...
    if ( IS_NPC( ch ) || number_percent( ) < ch->pcdata->learned[gsn_dragonpunch] )
    {
        damage( ch, victim, number_range( ch->level*6, (ch->level*15)+3 ), gsn_dragonpunch );
    }
    else
        send_to_char( C_DEFAULT, "You fail\n\r", ch);
        update_skpell( ch, gsn_dragonpunch );
...
void do_flare(CHAR_DATA *ch, char *argument)
{
  ...
  if (!can_use_skpell( ch, gsn_flare ) )
        { send_to_char(C_DEFAULT, "You think you know how to do that?\n\r", ch); 
        return;
        }
/* search for target, look two rooms away */
  if ( arg1[0] == '\0' )
  {
    send_to_char( C_DEFAULT, "Who do you want to flare?\n\r", ch);
    return;
  }
 in_room = ch->in_room;
  to_room = ch->in_room;
  ...
  if ( ( victim = ch->fighting ) == NULL )
    ...
    if ( arg2[0] == '\0' )
    {
      ...
      if ( dir == 6 )
      {
        send_to_char( C_DEFAULT, "Specify a direction?\n\r", ch );
        return;
      }
      if ( ( pexit = to_room->exit[dir] ) == NULL ||
           ( to_room = pexit->to_room ) == NULL )
      ...
      {
        send_to_char( C_DEFAULT, "Not a valid direction.\n\r",
                     ch );
        return;
      }
      ...
      if ( IS_SET( pexit->exit_info, EX_CLOSED ) )
      {
        send_to_char( C_DEFAULT, "Not through a door.\n\r",
ch );
        return;
      }
      ...
      if ( IS_SET( pexit->exit_info, EX_ISWALL ) )
      {
        send_to_char( C_DEFAULT, "Not through a wall.\n\r",
ch );
        return;
      }
    if ( number_percent() > ch->pcdata->learned[gsn_flare] +
(get_curr_dex( ch ) / 2)) {
        send_to_char(AT_RED,"You could not gather the heat!\n\r", ch );
        return;
        }
      for ( dist = 1; dist <= MAX_DIST; dist++ ) 
      {
               ...
               IS_SET( pexit->exit_info, EX_ISWALL) )
        {
          char_from_room( ch );
          char_to_room( ch, in_room );
          send_to_char(C_DEFAULT, "Your forces were dissapated before reaching your target./n/r",
                                 ch);
          return;
        }
      }
    ...
    if ( dist > 0 )
    {
      char_from_room( ch );
      char_to_room( ch, in_room );
      act( AT_WHITE, "A blast of flame flies in and hits $N!", victim,
          NULL, victim,  TO_NOTVICT );
      send_to_char( AT_WHITE, "A blast of flame flies in and hits you!",
victim);
      act( AT_WHITE,  "Your flare hits $N!", ch, NULL, victim, TO_CHAR );
      update_skpell( ch, gsn_flare );
      ranged_damage( ch, victim, ( ch->level*10) + ch->damroll,
gsn_flare);
...
void do_sandstorm(CHAR_DATA *ch, char *argument)
{
  ...
  if (!can_use_skpell( ch, gsn_flare ) )
        { send_to_char(C_DEFAULT, "You think you know how to do
that?\n\r", ch); 
        return;
        }
  ...
  if ( arg1[0] == '\0' )
  {
    send_to_char( C_DEFAULT, "Who do you want to sandblast?\n\r", ch);
    return;
  }
 in_room = ch->in_room;
  to_room = ch->in_room;
...
ch );
    ...
    if ( number_percent() > ch->pcdata->learned[gsn_flare] +
(get_curr_dex( ch ) / 2)) {
        send_to_char(AT_RED,"You could not gather the sand!\n\r", ch );
        return;
        }
      for ( dist = 1; dist <= MAX_DIST; dist++ ) 
      {
               ...
               IS_SET( pexit->exit_info, EX_ISWALL) )
        {
          char_from_room( ch );
          char_to_room( ch, in_room );
          send_to_char(C_DEFAULT, "Your forces were dissapated before
reaching your target./n/r",
                                 ch);
          return;
        }
      }
    ...
    if ( dist > 0 )
    {
      char_from_room( ch );
      char_to_room( ch, in_room );
      act( AT_WHITE, "A blast of sand flies up and hits $N!", victim,
          NULL, victim,  TO_NOTVICT );
      send_to_char( AT_WHITE, "A blast of sand flies up and hits you!",
victim);
      act( AT_WHITE,  "Your sandstorm hits $N!", ch, NULL, victim, TO_CHAR
);
      update_skpell( ch, gsn_sandstorm );
      ranged_damage( ch, victim, ( ch->level*5) + ch->damroll,
gsn_sandstorm);
...
void do_stance(CHAR_DATA *ch, char *argument)
{
    ...
    if ( arg[0] == '\0' )
	{
	send_to_char(AT_BLUE, "Syntax: stance <defensive/offensive/normal>\n\r", ch );
	}

    if (arg != 'defensive' || arg != 'normal' || arg != 'offensive')
	{
	send_to_char(AT_BLUE, "syntax: stance <defensive/normal/offensive>\n\r", ch );
	}

    if ( arg == 'defensive' && stc == stance_offensive)
	{
	ch->ac - 600;
	ch->hitroll - 100;
	ch->hitroll - mod;
	ch->damroll - 100;
	send_to_char(AT_BLUE, "You take up a defensive position.\n\r", ch);
	ch->stance = stance_defensive;
	}

    if ( arg == 'defensive' && stc == stance_normal)
	{
	ch->ac - 300;
	ch->hitroll - 100;
	ch->damroll - 50;
    ...
    if ( arg == 'normal' && stc == stance_defensive)
	{
	ch->ac + 300;
	ch->hitroll + 100;
	ch->damroll + 50;
	send_to_char(AT_BLUE, "You use your normal fighting stance.\n\r",
	ch);
	ch->stance = stance_normal
	}

    if ( arg == 'normal' && stc == stance_offensive)
	{
	ch->ac - 300;
	ch->hitroll - 100;
	ch->damroll - 50;
    ...
    if ( arg == 'offensive' && stc == stance_defensive)
	{
	ch->ac + 600;
	ch->hitroll + 100;
	ch->damroll + 50;
	send_to_char(AT_BLUE, "You poise yourself to strike.\n\r", ch);
	ch->stance = offensive;
	}

     if ( arg == 'offensive' && stc == stance_normal)
	{
	ch->ac + 300;
	ch->hitroll + 100;
	ch->damroll + 50;

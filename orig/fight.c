...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	{
    sprintf( buf1, "Your &gpoisoned&X %s &r%s&X $N%c (%d)",  attack, vp,
punct, dam );
    sprintf( buf2, "$n's &gpoisoned&X %s &G%s&X you%c (%d)", attack, vp,
punct, dam );
    sprintf( buf3, "$n's &gpoisoned&X %s &z%s&X $N%c (%d)",  attack, vp,
punct, dam );
    sprintf( buf4, "Your &gpoisoned&X %s &G%s&X you%c (%d)", attack, vp,
punct, dam );
    sprintf( buf5, "$n's &gpoisoned&X %s &z%s&X $n%c (%d)",  attack, vp,
punct, dam );
	}
	else
	{
	    sprintf( buf1, "Your %s &r%s&X $N%c (%d)",  attack, vp, punct,
dam );
	    sprintf( buf2, "$n's %s &G%s&X you%c (%d)", attack, vp, punct,
dam );
	    sprintf( buf3, "$n's %s &z%s&X $N%c (%d)",  attack, vp, punct,
dam );
	    sprintf( buf4, "Your %s &G%s&X you%c (%d)", attack, vp, punct,
dam );
	    sprintf( buf5, "$n's %s &z%s&X $n%c (%d)",  attack, vp, punct,
dam );
	}
    }

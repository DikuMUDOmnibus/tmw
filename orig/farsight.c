...
int far_room (CHAR_DATA *ch, const ROOM_INDEX_DATA *room,char *buf)
{
...
void do_farsight(CHAR_DATA *ch, char *argument)
{
  ...
  if ( number_percent( ) < ch->pcdata->learned[gsn_farsight] )  
    {
    sprintf (buf, "Right here you see:\n\r");
    if (far_room(ch,ch->in_room,buf) == 0)
        strcat (buf, "Noone\n\r");
...
dir_name[dir]);
            if (far_room(ch,pexit->to_room,buf)) /* if there is something
there */
                send_to_char (AT_WHITE,buf,ch);

         room = pexit->to_room; /* go to the next room */
         update_skpell( ch, gsn_farsight );
      } /* for distance */
    } /* for dir */
  }
}

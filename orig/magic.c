...
/*
    ...
    {
	send_to_char(C_DEFAULT, "But, you're in a mech!",ch);
	return;
    }
*/
    target_name = one_argument( argument, arg1 );
...
/*    bug("do_cast: bad return on sn %d", sn); */
    return;
}
...
/*
    if ( ( ( ch->level - 11 > victim->level ) 
      || ( ch->level + 11 < victim->level ) )
      && ( !IS_NPC(victim) ) )
    {
...
void spell_holysword( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    SET_BIT( obj->anti_class_flags, ITEM_ANTI_NINJA );
    send_to_char(AT_BLUE, "Ok.\n\r", ch );
    return;
}
...
void spell_remove_blessing( int sn, int level, CHAR_DATA *ch, void *vo  )
{
/* define some shit */
    OBJ_DATA *obj;
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    ...
    for ( iWear = 0; iWear < MAX_WEAR; iWear ++ )
    {
       ...
       if ( obj == get_eq_char( victim, WEAR_WIELD ))        {
         if ( IS_SET( obj->extra_flags, ITEM_POISONED )) {
            ...
            act(AT_BLUE, "The poisonous aura about $p fades.",  victim, obj, NULL, TO_CHAR );
            act(AT_BLUE, "$n's $p's poisonous aura fades.", victim, obj, NULL, TO_ROOM );
            yesno = TRUE;
          }
         if ( IS_SET( obj->extra_flags, ITEM_FLAME )) {
            REMOVE_BIT( obj->extra_flags, ITEM_FLAME );
            act(AT_BLUE, "The fiery aura about $p fades.",  victim, obj, NULL, TO_CHAR );
            act(AT_BLUE, "$n's $p's fiery aura fades.", victim, obj, NULL, TO_ROOM );
            yesno = TRUE;
          }
         if ( IS_SET( obj->extra_flags, ITEM_CHAOS )) {
            REMOVE_BIT( obj->extra_flags, ITEM_CHAOS );
            act(AT_BLUE, "The chaotic aura about $p fades.",  victim, obj, NULL, TO_CHAR );
            act(AT_BLUE, "$n's $p's chaotic aura fades.", victim, obj, NULL, TO_ROOM ); 
           yesno = TRUE;
          }
         if ( IS_SET( obj->extra_flags, ITEM_ICY )) {
            REMOVE_BIT( obj->extra_flags, ITEM_ICY );
            act(AT_BLUE, "The icy crust about $p melts.",  victim, obj, NULL, TO_CHAR );
            act(AT_BLUE, "$n's $p's icy crust melts.", victim, obj, NULL, TO_ROOM ); 
           yesno = TRUE;
          }         
             if (yesno == TRUE) {
            unequip_char( victim, obj );
...
void spell_entangle( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    sprintf( buf, "%s creates a force to hold you in place.\n\r", ch->name );
    send_to_char(AT_GREEN, buf, victim );
    act(AT_GREEN, "$n is held in place.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| IS_SET( victim->act2, PLR_NOSUMMON )
	|| victim->fighting
...
/*    send_to_char( AT_BLUE, "You feel a wave of nausia come over
you.\n\r", ch );
...
/* take out the below statement if spellcraft crashes mud */
/* statement above, kjodo */
...
/* Marquoz detect doors spell.  Thi/Mag spell. */
/* whoo go marquoz, this is also the first example
   of how to do affects so that i don't need aff3 */

void spell_awareness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    AFFECT_DATA af;
    
    if( is_affected( ch, gsn_awareness ))
    {
        send_to_char( C_DEFAULT, "You already have a heightened awareness.\n\r", ch );
        return;
    }
    ...
    send_to_char( C_DEFAULT, "Your eyes tingle.\n\r", ch );
    return;
}    

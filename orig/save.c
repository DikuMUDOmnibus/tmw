...
/* 
    So rooms can save things-- Written by Sledge -- Added by ToR  
...
@@comment        if ( obj->item_type == ITEM_BUILDING_OBJECT )
            continue;
    ...
    for ( ; mob != NULL; mob = mob->next_in_room )
    {
        ...
        if ( mob->carrying != NULL )
        {
            ...
            for ( obj = mob->carrying; obj != NULL; obj =
obj->next_content )
            {
            ...
            {
                for ( obj = mob->wearing[i]; obj != NULL; obj =
obj->next_worn )
                    fwrite_obj( mob, obj, fp, 0, FALSE );
            }
        }
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    fprintf( fp, "Clvl        %d\n",    ch->clev                );
    fprintf( fp, "Ctmr        %d\n",    ch->ctimer              );
    fprintf( fp, "Clslvls     %d %d %d %d %d %d %d %d %d %d\n",
             ch->class_level[0], ch->class_level[1], ch->class_level[2],
             ch->class_level[3], ch->class_level[4], ch->class_level[5],
             ch->class_level[6], ch->class_level[7], ch->class_level[8],
             ch->class_level[9] );
		 ...
		 bool storage )
{
    ...
    case ITEM_MECH:
        if (obj->value[1] > 0)
        {
         fprintf(fp, "DamageMAX   %d\n", obj->value[1]);
        }
	if (obj->value[2] > 0)
	{
	 fprintf(fp, "ShieldMAX   %d\n", obj->value[2]);
	}
	if (obj->value[3] > 0)
	{
	 fprintf(fp, "TonnageMAX  %d\n", obj->value[3]);
	}
	if (obj->value[4] > 0)
	{
	 fprintf(fp, "ArmorType   %s\n", 
	...
	break;
      ...
      if ( obj->value[1] > 0 )
	{
	  fprintf( fp, "Max Passengers   %d\n", obj->value[1]);
		   }
	  if (obj->value[2] > 0 )
	    {
	      fprintf( fp, "Movement Type  %s\n",
	   ...
	   if (obj->value[4] > 0 )
             {
		  fprintf( fp, "Max Weapons    %d\n", obj->value[4]);
		     }
	   if (obj->value[5] > 0 )
	     {
	       fprintf( fp, "Max Armor     %d\n", obj->value[5]);
			}
	       break;
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'C':
		...
		break;
            ...
            if (!str_cmp( word, "Clslvls" ) )
            {
		ch->class_level[0] = fread_number( fp );
                ch->class_level[1] = fread_number( fp );
                ch->class_level[2] = fread_number( fp );
                ch->class_level[3] = fread_number( fp );
                ch->class_level[4] = fread_number( fp );
                ch->class_level[5] = fread_number( fp );
                ch->class_level[6] = fread_number( fp );
                ch->class_level[7] = fread_number( fp );
                ch->class_level[8] = fread_number( fp );
                ch->class_level[9] = fread_number( fp );
		fMatch = TRUE;
		break;
            }
	    break;
	...
	case 'D':
		...
		KEY( "Dimension", 	ch->dimension,		fread_number( fp ) );
...
/*	        if ( ch->trust > 107 && ch->level < 108 && !IS_CODER(ch) )
	...
	case 'H':
	    ...
	    KEY( "Incarnat",	ch->incarnations,	fread_number(fp));
            break;
	case 'K':
	    KEY( "Kingdom",	ch->kingdom,		fread_number( fp ));
	    break;
	case 'L':
	...
	case 'M':
	    KEY( "Mech",	ch->pcdata->in_mech,	fread_string(fp));
	    KEY( "MechDesc",	ch->pcdata->mdesc,	fread_string(fp));
	    break;
...
void fread_obj( CHAR_DATA *ch, FILE *fp, bool storage )
{
    ...
    *obj		= obj_zero;
	...
	case 'D':
		...
		KEY( "Dimension", 	obj->dimension,		fread_number( fp ) );
	    break;
...
/*  fprintf( fp, "&CPlan: &W%s\n",			ch->pcdata->plan );
    */
  fprintf( fp, "&CPkills: &W%d     ", ch->pkills );
  fprintf( fp, "&CPdeaths: &W%d\n", ch->pkilled );
  fprintf( fp, "&CArena wins: &W%d  ", ch->arenawon );
...
{  char
*finger;  char  buf[MAX_STRING_LENGTH];  /*CHAR_DATA  *wch;*/  /*CHAR_DATA
*victim;*/ /* sprintf( buf, "          Finger Info
\n\r" );  send_to_char( AT_WHITE, buf, ch );      sprintf( buf,
"          ------ ----\n\r\n\r" );  send_to_char( AT_WHITE, buf, ch );  */
sprintf( buf, "\n\r      &CInfo on &W%s\n\r\n\r", name);  send_to_char(
AT_WHITE, buf, ch );           finger = fread_string( fp );  sprintf( buf,
"%s", finger );  send_to_char( AT_WHITE, buf, ch );  send_to_char(
AT_WHITE, "\n\r", ch );  return;}

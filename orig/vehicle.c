...
*/
   ...
   if (vehicle->value[1] != 3) 
    ...
    if (vehicle->value[1] == 1)
     {
sprintf(buf, "%s slowly set's sail to the %s.", capitalize(ch->in_room->inside_of->short_descr), dir_name[door]);
     }
    /* Land Vehicles */
    ...
    {
    sprintf(buf, "%s mysteriously floats %s.", capitalize(ch->in_room->inside_of->short_descr), dir_name[door]);
    }
    /* Golems */
    else if (vehicle->value[1] == 4)
    {
    sprintf(buf, "%s angrily stomps to the %s.", capitalize(ch->in_room->inside_of->short_descr), dir_name[door]);
    }
    /* If new and not set, display SOMETHING! */
    else
    {
    sprintf(buf, "%s moves to the %s.", capitalize(ch->in_room->inside_of->short_descr), dir_name[door]);
    }


    for ( vch = ch->in_room->inside_of->in_room->people; vch ; vch = vch->next_in_room )
    {
    ...
    obj_to_room( obj, to_room );
     ...
     sprintf(buf, "%s slowly sails in.", capitalize(ch->in_room->inside_of->short_descr));
     }  
    /* Land Vehicles */
    else if (vehicle->value[1] == 2)
    {
    ...
    {
    sprintf(buf, "%s mysteriously floats in.", capitalize(ch->in_room->inside_of->short_descr));
    }
    /* Golems */
    else if (vehicle->value[1] == 4)
    {
    sprintf(buf, "The ground shakes as %s stomps in.", capitalize(ch->in_room->inside_of->short_descr));
    }
       /* If new and not set, display SOMETHING! */
    else
    {
    sprintf(buf, "%s enters the room.", capitalize(ch->in_room->inside_of->short_descr));
    }
    ...
    for ( vch = ch->in_room->people; vch ; vch = vch->next_in_room )
{
   if (!IS_SET(vch->act2, PLR_LOOKOUT ) && !IS_SET(vch->act2, PLR_DRIVER))
    ...
    {
    send_to_char( "You barely feel the ship sail through the air smoothly.\n\r", vch );
    }
    /* Golems - There should be no passangers in Golems */
    else if (vehicle->value[1] == 4)
    {
    send_to_char( "This message is an Error, Please contact an imm about this Golem.\n\r", vch );
...
(IS_SET(pexit->exit_info, EX_CLOSED)))
            ...
            if (hack_scan_room(ch,pexit->to_room,buf)) /* if there is something there */
                send_to_char (buf,ch);
...
/*	smash_tilde( argument ); */
   ...
   if ( arg[0] == '\0' || argument[0] == '\0' )
	...
	{
	send_to_char("That person was trained as a captain, they can't be your crew!\n\r", ch );
	return;
	}
...
/* Okay, Finally all the checks are done, victim is valid */
   ...
   /* Lookout */
	...
	REMOVE_BIT( victim->act2, PLR_HELM);
     ...
     if ( IS_SET( victim->act2, PLR_LOOKOUT ) )
        ...
        else
        {
            ...
            send_to_char("You have been appointed Lookout by your Captain\n\r", victim );
            send_to_char( "Okay\n\r", ch );
         return;
        }
    }
	...
	REMOVE_BIT(victim->act2, PLR_LOOKOUT);
        ...
        else
        {
            ...
            send_to_char("You have been appointed helmsman by your Captain\n\r", victim );
            send_to_char( "Okay\n\r", ch );
         return;
        }
    }
	...
	return;
     ...
     if ( IS_SET( victim->act2, PLR_DRIVER ) )
        ...
        else
        {
            ...
            send_to_char("You have been appointed Driver by your Captain\n\r", victim );
            send_to_char( "Okay\n\r", ch );
         return;
        }
    }
 ...
 /* We'll hack PLR_DRIVER since they do the same thing */
     if ( IS_SET( victim->act2, PLR_DRIVER ) )
        ...
        {
            SET_BIT( victim->act2, PLR_DRIVER );
            send_to_char("You have been appointed pilot by your Captain\n\r", victim );
            send_to_char( "Okay\n\r", ch );
         return;
        }
    }
...
/*   char	     buf[MSL]; */
   ...
   if (!IS_SET(ch->act2, PLR_MANCANNON) && !IS_IMMORTAL(ch) )
 {
  ...
  cball = get_eq_char( ch, WEAR_HOLD );

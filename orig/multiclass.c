...
char *class_levels( CHAR_DATA *ch, bool pSave )
{
  static char buf[ 512 ];
  char buf2[ 10 ];
  int iClass;
  ...
  for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
	{
	strcat( buf, " " );
	sprintf( buf2, "%d", ch->class_level[iClass] );
	strcat( buf, buf2 );
	}
  if ( pSave )
    strcat( buf, " -1" );
...
|| ( IS_ANTI_CLASS( obj, ITEM_ANTI_NINJA  ) && is_class( ch, CLASS_NINJA ) )
|| ( IS_ANTI_CLASS( obj, ITEM_ANTI_AIRKIN  ) && is_class( ch, CLASS_AIRKIN ) )
|| ( IS_ANTI_CLASS( obj, ITEM_ANTI_EARTHKIN) && is_class( ch, CLASS_EARTHKIN ) )
|| ( IS_ANTI_CLASS( obj, ITEM_ANTI_FIREKIN) && is_class( ch, CLASS_FIREKIN ) )
|| ( IS_ANTI_CLASS( obj, ITEM_ANTI_WATERKIN) && is_class(ch, CLASS_WATERKIN ) ) */
...
|| ( IS_ANTI_RACE( obj, ITEM_ANTI_KREEN     ) && ch->race == RACE_KREEN ) 
|| ( IS_ANTI_RACE( obj, ITEM_ANTI_ELEMENTAL ) && ch->race == RACE_ELEMENTAL ) ) 
  return TRUE;
return FALSE;
}

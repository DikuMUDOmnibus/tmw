...
bool redit_show( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( pexit = pRoom->exit[door] ) )
	{
	    ...
	    {
            sprintf( buf, "&W-%-5s to &z[&W%5d&z] &WSDC: &z[&W%5d&z]",
                capitalize(dir_name[door]),
                pexit->to_room ? pexit->to_room->vnum : 0,
                pexit->sdc );
            strcat( buf1, buf );
 ...
 */
	...
	/* Create the room.*/
/*	Commented out, gonna let the following link code handle
	everything after room is made.
    ...
    if ( !str_cmp( command, "sdc" ) )
    {

        if ( arg[0] == '\0' || !is_number( arg ) )
        {
            send_to_char(C_DEFAULT, "Syntax:  [direction] sdc [ammount]\n\r", ch );
            return FALSE;
        }

	if (!(IS_SET(pRoom->exit[door]->exit_info, EX_ISWALL)))
	{
	send_to_char(AT_WHITE, "That is not a wall.\n\r", ch);
	return FALSE;
	}

        if ( !pRoom->exit[door] )
            pRoom->exit[door] = new_exit();
        pRoom->exit[door]->sdc = atoi( arg );

        send_to_char(C_DEFAULT, "Exit SDC set.\n\r", ch );
        return TRUE;
    }
...
/*
    sprintf( buf, "You are now in room: %d\n\r", value);
    send_to_char( C_DEFAULT, buf, ch );
    char_from_room( ch );
    char_to_room(ch, get_room_index( value ));
*/
    return TRUE;
}
...
void show_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *obj )
{
  ...
  case ITEM_MECH:
    sprintf(buf,
     "&z[&Wv1&z] &cDamage Maximum&w:  &z[&R%d&z]\n\r"
     "&z[&Wv2&z] &cShield Maximum&w:  &z[&R%d&z]\n\r"
     "&z[&Wv3&z] &cTonnage Maximum&w: &z[&R%d&z]\n\r"
     "&z[&Wv4&z] &cArmor Type&w:      &z[&W%s&z]\n\r",
     obj->value[1],obj->value[2],obj->value[3],
     flag_string(mecharmor_flags, obj->value[4]));
    send_to_char(C_DEFAULT,buf,ch);
    break;
	...
	case ITEM_PHONE:
		sprintf( buf,
			"[v1] Type: (1 cell 2 hardline) [%d]\n\r"
			"[v2] Number (7 digits! no dash)[%d]\n\r",
			obj->value[1],
			obj->value[2]);
      send_to_char(C_DEFAULT, buf, ch);
      break;
    ...
    case ITEM_VEHICLE:
      sprintf( buf,
		  ...
		  "[v7] Locked?:          [%d]\n\r",
	       obj->value[1],
               flag_string(movement_flags, obj->value[2]),
      ...
      break;
        ...
        case ITEM_CLIP:
            sprintf( buf,
                "&z[&Wv1&z] &CCapacity&W:		&z[&W%d&z]&X\n\r"
                "&z[&Wv2&z] &CCalibur&W:		&z[&W%d&z]&X\n\r",
        ...
        case ITEM_BULLET:
            sprintf( buf,
                "&z[&Wv1&z] &CBullet Type&W:		&z[&W%s&z]%d&X\n\r"
                "&z[&Wv2&z] &CCalibur&W:		&z[&W%d&z]&X\n\r",
		flag_string( weapon_flags, obj->value[1] ),obj->value[1], obj->value[2] );
           send_to_char(C_DEFAULT, buf, ch );
         break;
...
#else
            sprintf( buf, "&z[&Wv0&z] &cCredits&w:   &z[&R%d&z]\n\r",
		     obj->value[0] );
#endif
	    send_to_char(C_DEFAULT, buf, ch );
	    break;
...
bool set_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *pObj, int value_num, char *argument)
{
	...
	case ITEM_PILL:
        ...
        case ITEM_BULLET:
            switch( value_num )
            {
             ...
             case 1:
	            ...
	            pObj->value[1] = flag_value( weapon_flags, argument );
                    break;
             case 2:
        ...
        case ITEM_BOMB:
            ...
            {
             default:
              do_help( ch, "HELP_BOMB" );
              return FALSE;
             case 1:
                    send_to_char(C_DEFAULT, "BOMB TYPE SET.\n\r\n\r", ch );
                    pObj->value[1] = atoi( argument );
                    break;
             case 2:
                    send_to_char(C_DEFAULT, "BLAST RADIUS SET.\n\r\n\r", ch );
                    pObj->value[2] = atoi( argument );
                    break;
             case 3:
                    send_to_char(C_DEFAULT, "BOMB RATING SET.\n\r\n\r", ch );
        ...
        case ITEM_PHONE:
            switch( value_num )
            {
             ...
             case 1:
                    ...
                    break;
             case 2:
                    send_to_char(C_DEFAULT, "PHONE NUMBER SET.\n\r\n\r", ch );
                    pObj->value[2] = atoi( argument );
                    break;
             }
             break;
	...
	{
	default:
	  do_help(ch, "HELP_VEHICLE");
	  return FALSE;
        case 1:
	  send_to_char(C_DEFAULT, "MAX PASSENGERS SET.\n\r\n\r", ch);
          pObj->value[1] = atoi( argument );
          break;
        case 2:
          send_to_char(C_DEFAULT, "MOVEMENT TYPE SET.\n\r\n\r", ch);
          pObj->value[2] = flag_value( movement_flags, argument);
          break;
        case 3:
          send_to_char(C_DEFAULT, "GUNNER SET.\n\r\n\r", ch);
          pObj->value[3] = atoi( argument );
          break;
        case 4:
          send_to_char(C_DEFAULT, "MAX WEAPONS SET.\n\r\n\r", ch);
          pObj->value[4] = atoi( argument );
          break;
        case 5:
          send_to_char(C_DEFAULT, "MAX ARMOR SET.\n\r\n\r", ch);
          pObj->value[5] = atoi( argument );
          break;
		case 6:
          send_to_char(C_DEFAULT, "KEY VNUM SET.\n\r\n\r", ch);
          pObj->value[6] = atoi( argument );
          break;
		case 7:
          send_to_char(C_DEFAULT, "LOCKED SET.\n\r\n\r", ch);
      ...
      break;
        ...
        case ITEM_RANGED_WEAPON:
            switch( value_num )
            {
             ...
             case 1:
                    ...
                    break;
             case 2:
                    send_to_char(C_DEFAULT, "CALIBUR SET.\n\r\n\r", ch );
                    pObj->value[2] = atoi( argument );
                    break;
             case 3:
                    ...
                    break;
             case 4:
                    send_to_char(C_DEFAULT, "AUTOMATIC SHOTS PER ROUND SET.\n\r\n\r", ch );
                    pObj->value[4] = atoi( argument );
                    break;
             }
             break;
        ...
        case ITEM_PARMOR:
            ...
            {
             default:
              do_help( ch, "HELP_POWER_ARMOR" );
              return FALSE;

             case 1:
                    send_to_char(C_DEFAULT, "DAMAGE SOAK SET.\n\r\n\r", ch );
                    pObj->value[1] = atoi( argument );
                    break;
             case 2:
                    send_to_char(C_DEFAULT, "MAX MOUNTS SET.\n\r\n\r", ch );
                    pObj->value[2] = atoi( argument );
                    break;
 ...
 case ITEM_MECH:
  ...
  {
   default:
    do_help(ch, "OEDIT_ITEM_MECH");
    return FALSE;
   case 1:
    send_to_char(C_DEFAULT, "DAMAGE MAX SET.\n\r\n\r", ch);
    pObj->value[1] = atoi(argument);
    break;
   case 2:
    send_to_char(C_DEFAULT, "SHIELD MAX SET.\n\r\n\r", ch);
    pObj->value[2] = atoi(argument);
    break;
   case 3:
    send_to_char(C_DEFAULT, "TONNAGE MAX SET.\n\r\n\r", ch);
    pObj->value[3] = atoi(argument);
    break;
   case 4:
    send_to_char(C_DEFAULT, "ARMOR TYPE SET.\n\r\n\r", ch);
    pObj->value[4] = flag_value(mecharmor_flags, argument);
    break;
  }
  break;
	...
	case ITEM_MONEY:
	    switch ( value_num )
	    {
		    ...
		    do_help( ch, "ITEM_MONEY" );
	            return FALSE;
	        case 0:
	            send_to_char(C_DEFAULT, "CREDITS AMOUNT SET.\n\r\n\r", ch );
	            pObj->value[0] = atoi( argument );
#ifdef NEW_MONEY
...
bool oedit_show( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char(C_DEFAULT, buf, ch);
    sprintf( buf, "&cDimension&w:&z   [&R%d&z]\n\r", pObj->dimension );
    send_to_char(C_DEFAULT, buf, ch);
...
bool oedit_dimension( CHAR_DATA *ch, char *argument )
{
    OBJ_INDEX_DATA *pObj;
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char(C_DEFAULT, "Syntax:  dimension [number]\n\r", ch );
	return FALSE;
    }

    pObj->dimension = atoi( argument );

    send_to_char(C_DEFAULT, "Dimension set.\n\r", ch);
    return TRUE;
}
...
bool medit_show( CHAR_DATA *ch, char *argument )
{
   ...
   sprintf( buf,
    ...
    send_to_char(C_DEFAULT, buf, ch);
#endif
	sprintf(buf,  "&cDimension  &w[&R%d&w]\n\r", pMob->dimension );
	send_to_char( C_DEFAULT, buf, ch );
...
bool medit_gold( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char(C_DEFAULT, "Syntax:  credits [amount]\n\r", ch );
	return FALSE;
    }
#ifdef NEW_MONEY
    ...
    send_to_char(C_DEFAULT, "Credits set.\n\r", ch);
    return TRUE;
}
...
bool medit_dimension( CHAR_DATA *ch, char *argument )
{
    MOB_INDEX_DATA *pMob;
...
void do_cube( CHAR_DATA *ch, char *argument )
{
/*variables*/
AREA_DATA *pArea;
ROOM_INDEX_DATA *pRoom;
...
/*syntax: cube <height> <rows> <columns> <first vnum> */
if (arg1[0] == '\0' || !is_number(arg1) ||
     arg2[0] == '\0' || !is_number(arg2) ||
     arg3[0] == '\0' || !is_number(arg3) ||
     arg4[0] == '\0' || !is_number(arg4))
{
send_to_char(AT_WHITE, "Syntax: cube <height> <rows> <columns> <first vnum>\n\r", ch);
return;
}
...
columns     = atoi(arg3);
NewVnum  = vnum;
topvnum    = (vnum+(height*rows*columns))-1;

/*check if vnums going to be used are taken */

    if (get_vnum_area(vnum))
    vnumused = TRUE;
if (get_vnum_area(topvnum))
vnumused = TRUE;

if (vnumused)
{
send_to_char(AT_WHITE, "Some of the vnums this cube would encompass are taken.\n\r", ch );
return;
}
             /* create the new area */
...
/* start the creation/linking process */
send_to_char(AT_WHITE, "Creating/linking...\n\r", ch );

for ( NewHeight = 0; NewHeight < height; NewHeight++ )
{
    for ( NewColumn = 0; NewColumn < columns; NewColumn++ )
    {
        for ( NewRow = 0; NewRow < rows; NewRow++ )
        {
         ...
         char_to_room( ch, ch->desc->pEdit );
          ...
          if (NewRow > 0)
          {
          /* link vnum(vnum) north vnum(vnum - 1) */

sprintf( buf, "link %d", NewVnum -1 );
change_exit( ch, buf, DIR_NORTH );

          }
          if (NewColumn > 0)
          {
          /* link vnum(vnum) west vnum(vnum - rows)*/
sprintf( buf, "link %d", NewVnum - rows );
change_exit( ch, buf, DIR_WEST );

          }
          if (NewHeight > 0)
          {
          /* link vnum(vnum) down vnum(vnum - (rows*columns)) */

sprintf( buf, "link %d", NewVnum - (rows*columns));
change_exit( ch, buf, DIR_DOWN );

...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
         "&BRace: &R %s  &B Age:&R %d&G(&R%dhrs&G)&B  Vocation:&R%s\n\r",
    ...
    sprintf( buf,
         "&BStr:&R%d&G(&Y%d&G)  &CAutoExit :(%s)  &BHit Pts:&R%d&Y/&G%d\n\r",
    ...
    sprintf( buf,
         "&BInt:&R%d&G(&Y%d&G)  &CAutoLoot :(%s)  &BWill Power:&R%d&Y/&G%d\n\r",
    ...
    sprintf( buf,
          "&BWis:&R%d&G(&Y%d&G)  &CAutoSac  :(%s)  &BMove Pts:&R%d&Y/&G%d\n\r",
    ...
    sprintf( buf,
          "&BDex:&R%d&G(&Y%d&G)  &CAutoCoins:(%s)  &BExp: &R%d\n\r",
    ...
    sprintf( buf,
         "&BCha:&R%d  &BSaving Throw:&R%d\n\r",
         IS_NPC(ch) ? 20 : ch->charisma,
         ( ch->race != RACE_DWARF ) ? ch->saving_throw : ch->saving_throw + ch->saving_throw * 25 / 100 ); 
    send_to_char(AT_CYAN, buf, ch );
    
    send_to_char( D,"&B==========================================\n\r", ch);
    send_to_char( AT_BLUE, "Class levels:\n\r", ch );
    sprintf( buf, "&BOpr: &C%d &BHlr: &C%d &BRog: &C%d &BSdr: &C%d &BWth: &C%d\n\r",
    ch->class_level[0], ch->class_level[1], ch->class_level[2], ch->class_level[3],
    ch->class_level[4] );
    send_to_char( C_DEFAULT, buf, ch);
    sprintf( buf, "&BAgt: &C%d &BPil: &C%d &BHck: &C%d &BTch: &C%d &BVam: &C%d\n\r",
    ch->class_level[5], ch->class_level[6], ch->class_level[7],
    ch->class_level[8], ch->class_level[9] );
    send_to_char( C_DEFAULT, buf, ch );
    send_to_char( D,"&B==========================================\n\r", ch);
    sprintf( buf,
         "&BItems: %d/(%d)  &BWeight: %d/(%d)\n\r",
         ch->carry_number, can_carry_n( ch ),
         ch->carry_weight, can_carry_w( ch ) );
    send_to_char(AT_CYAN, buf, ch );

    sprintf( buf,
         "&BWar Pts: &R%d  &BArenaW: &R%d  &BArenaL: &R%d  &BPkills: &R%d &BPkilled: &R%d\n\r",
      ...
      switch ( ch->position ) 
 {
    ...
    */
    ...
    send_to_char( AT_WHITE, buf, ch );
    sprintf( buf, "Wimpy set to&R %d&B hit points.\n\r", ch->wimpy );
    send_to_char( AT_BLUE, buf, ch );
    if ( !IS_NPC( ch ) )
    {
...
send_to_char(C_DEFAULT,
"&z==&O=&z=========&O==&z===========&O=&z=======&O=&z==&O=&z======&O==&z========&O=&z========&O=&z===\n\r \n\r",ch);

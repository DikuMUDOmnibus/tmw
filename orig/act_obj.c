...
/*  int tot_coins; */
	...
	if ( obj->item_type == ITEM_BOMB && obj->timer > 0 ) {
		char buf[MAX_STRING_LENGTH];
		if ( number_percent( ) < ch->pcdata->learned[gsn_disarming] ) {
			sprintf(buf, "You successfully disarm %s.\n\r", obj->short_descr);
			send_to_char( AT_WHITE, buf, ch );
			update_skpell( ch, gsn_disarming );
			obj->timer = -1;
		}
		else {
			sprintf(buf, "You attempt to disarm %s and fail miserably.\n\r", obj->short_descr);
...
*/
	...
	amount = obj->value[0];
	if ( amount > 1 )
          sprintf( buf, "You counted %d credits.\n\r", amount );
	else
	  sprintf( buf, "You counted 1 credit.\n\r" );
	send_to_char(AT_YELLOW, buf, ch );
        ch->gold += amount;
...
/* use three arguments */
/* syntax: typhoon <target> <direction> */
  argument = one_argument( argument, arg1 );
  argument = one_argument( argument, arg2 );

  if (!can_use_skpell( ch, gsn_tornado ) )
...
/* search for target, look two rooms away */
  if ( arg1[0] == '\0' )
  {
    send_to_char( C_DEFAULT, "Who do you want to tornado?\n\r", ch);
    return;
  }
  in_room = ch->in_room;
  to_room = ch->in_room;
...
ch );
    ...
    if ( number_percent() > ch->pcdata->learned[gsn_tornado] +
(get_curr_dex( ch ) / 2)) {
        send_to_char(AT_RED,"You could not gather the wind!\n\r", ch );
        return;
        }
      for ( dist = 1; dist <= MAX_DIST; dist++ )
      {
               ...
               IS_SET( pexit->exit_info, EX_ISWALL) )
        {
          ...
          send_to_char(C_DEFAULT, "Your forces were dissapated before reaching your target.\n\r",
                                 ch);
          return;
        }
      }
    ...
    if ( dist > 0 )
    {
      char_from_room( ch );
      char_to_room( ch, in_room );
      act( AT_WHITE, "A tornado flys in and hits $N!", victim, 
          NULL, victim,  TO_NOTVICT );
      send_to_char( AT_WHITE, "A tornado flys in and hits you!", victim);
      act( AT_WHITE,  "Your tornado hit $N!", ch, NULL, victim, TO_CHAR );
      update_skpell( ch, gsn_tornado );
/* remove some char's eq -- scatter in room */
    for ( iWear = 0; iWear < MAX_WEAR; iWear ++ )
        ...
        if (number_range( 1, 5 ) > 2 )
          {
          act(C_DEFAULT, "$p has been ripped off!", victim,  obj, NULL, TO_CHAR );
...
/* Change the parameters of create_money to MONEY_DATA *amount */
	...
	amount   = atoi( arg );
	argument = one_argument( argument, arg );
		if ( amount <= 0 || str_prefix( arg, "credits" ) )
	{
	...
	if ( ch->gold < amount )
	{
	    send_to_char(AT_DGREEN, "You haven't got that many credits.\n\r", ch );
	    return;
	}
	...
	send_to_char(AT_YELLOW, "OK.\n\r", ch );
	act(AT_YELLOW, "$n drops some credits.", ch, NULL, NULL, TO_ROOM );
	return;
    }
...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	int amount;
	amount   = atoi( arg1 );
	if ( amount <= 0 || str_prefix( arg2, "credits" ) )
	{
	    send_to_char(AT_DGREEN, "Sorry, you can't do that.\n\r", ch );
	    return;
	}
	...
	if ( ch->gold < amount )
	{
	    send_to_char(AT_YELLOW, "You haven't got that many credits.\n\r", ch );
	    return;
	}

	ch->gold     -= amount;
	victim->gold += amount;
	sprintf( buf, "You give %s %d credits.", victim->name, amount );
	act(AT_YELLOW, buf, ch, NULL, victim, TO_CHAR );
	sprintf( buf, "%s gives you %d credits.", 
           can_see( victim, ch ) ? ch->name : "someone", amount );
	act(AT_YELLOW, buf, ch, NULL, victim, TO_VICT );
	act(AT_YELLOW, "$n gives $N some credits.",  ch, NULL, victim, TO_NOTVICT );
...
*/	
	...
	if ( obj->value[1] <= 0 )
	{
	    send_to_char(AT_BLUE, "It seems to be empty now.\n\r", ch );
	    return;
	}
	break;
    }
...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace )
{
	...
	if (inv->deleted)
	continue;
	if (IS_SET(inv->extra_flags, ITEM_LOADED ))
	{
	sprintf( buf, "%s automatically being unmounted.\n\r",
                 inv->short_descr );
	send_to_char(AT_WHITE, buf, ch );
	REMOVE_BIT(inv->extra_flags, ITEM_LOADED);
	}
	}
    }
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
   ...
   if ( !IS_NPC( ch ) && get_curr_str( ch ) < ( get_obj_weight( obj ) - 10 ) )
   {
...
void do_third( CHAR_DATA *ch, char *argument )
{
    ...
    bool      fReplace;
    int       weapon_type = 0;
    OBJ_DATA *objc = get_eq_char( ch, WEAR_WIELD_2);
    ...
    if ( !get_eq_char( ch, WEAR_WIELD_2))
	{
	send_to_char(AT_BLUE, "You should try dual wielding first./n/r", ch );
	return;
	}
   ...
   if ( CAN_WEAR( obj, ITEM_WIELD ) )
	...
	if ( get_obj_weight( obj ) >= get_obj_weight( objc ))
	    {
	    send_to_char(AT_BLUE, "It must be of less weight than you dual wielded weapon./n/r", ch );
	    return;
	    }
...
void do_mount( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char(AT_BLUE, "Mount what?\n\r", ch );
        return;
    }
	...
	{
	send_to_char( AT_BLUE, "That's already mounted.\n\r", ch );
	return;
	}
	if ( obj->item_type != ITEM_RANGED_WEAPON )
	{
	send_to_char( AT_BLUE, "Mounting guns on your armor works much better.\n\r", ch );
	return;
	}
	if ( IS_AFFECTED2( ch, AFF_RAGE ) )
        {
         send_to_char( AT_RED, "You are too enraged to mount a weapon.\n\r", ch );
         return;
        }

    for ( mounted = ch->carrying; mounted; mounted = mounted->next_content )
    {
	if (mounted->deleted)
	continue;
	if (IS_SET(mounted->extra_flags, ITEM_LOADED))
	{
		count++;
	}
    }

	if ((parmor = parmor_check( ch )) == NULL )
	{
	send_to_char(AT_BLUE, "You need a power armor suit to mount weapons on.\n\r", ch );
	return;
	}
	if (count >= parmor->value[2])
	{
	send_to_char(AT_BLUE, "You have already mounted the max guns on this power armor\n\r", ch );
	return;
	}

        SET_BIT(obj->extra_flags, ITEM_LOADED);
		send_to_char( AT_BLUE, "It is now mounted.\n\r", ch );


    return;
}

void do_unmount( CHAR_DATA *ch, char *argument )
{
    OBJ_DATA *obj;
    char      arg [ MAX_INPUT_LENGTH ];
    ...
    if ( arg[0] == '\0' )
    {
        ...
        if ( !( obj = get_obj_carry( ch, arg ) ) )
        {
            send_to_char(AT_BLUE, "You don't even HAVE one of those!.\n\r", ch );
            return;
        }

        if ( IS_SET(obj->extra_flags, ITEM_LOADED) )
        {
        REMOVE_BIT(obj->extra_flags, ITEM_LOADED);
                send_to_char( AT_BLUE, "It is now unmounted.\n\r", ch );
		}

    return;
}

OBJ_DATA * parmor_check( CHAR_DATA *ch )
{
  OBJ_DATA *obj;
...
void do_wear( CHAR_DATA *ch, char *argument )
{
        ...
        for ( obj = ch->carrying; obj; obj = obj_next )
	{
	    obj_next = obj->next_content;
         ...
         if ( IS_SET( obj->wear_flags, WEAR_WIELD ) )
	  continue;
         ...
         if ( IS_SET( obj->wear_flags, WEAR_WIELD ) )
	{
	 send_to_char(AT_BLUE,
          "Use the wield or dual commands for wielded items.\n\r",
           ch );
	 return;
	}
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    iname = number_range( 0 , 2 ); /* change second # if you add/delete
				       immortal names */
    one_argument( argument, arg );
	...
	send_to_char( AT_WHITE,
        ...
        act( AT_WHITE, 
             "$n offers $mself to Kjodo, who graciously declines.",
	...
	if ( obj->item_type == ITEM_BOMB && obj->timer != -1 ) {
		send_to_char( AT_WHITE, "Are you trying to blow the gods up?\n\r", ch );
		return;
	}
	...
	sprintf( buf1, "%d credit%s", award, (award != 1) ? "s" : "" );
#ifdef NEW_MONEY
...
void do_quaff( CHAR_DATA *ch, char *argument )
{
    ...
    if ( number_percent() < 35 && ch->fighting )
    {
	send_to_char(AT_WHITE, "You spill the potion!", ch );
	extract_obj( obj );
        return;
    }
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( abs(ch->level - victim->level) > 10 && ( !IS_NPC(victim) ) )
    {
...
/*	|| !str_cmp   ( arg1, "gold"  ) )  */
	...
	if ( amount <= 0 )
	{
	    send_to_char(AT_BLOOD, "You couldn't get any credits.\n\r", ch );
	    return;
	}

	ch->gold     += amount;
	victim->gold -= amount;
	sprintf( buf, "Jackpot!  You got %d credits.\n\r", amount );
	send_to_char(AT_RED, buf, ch );
        update_skpell( ch, gsn_steal );
	return;
    }
...
void do_list( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->in_room->vnum == ROOM_VNUM_ARTIFACTOR )
    {
      ...
      send_to_char( AT_BLUE, "25000 credits per item level.\n\r", ch );
      send_to_char( C_DEFAULT, "Indestructable <item>\n\r", ch );
      send_to_char( AT_WHITE, "\n\rRemake item:\n\r", ch );
      send_to_char( AT_BLUE, "100000 credits.\n\r", ch);
...
void do_invoke( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) )
      return;
/*
    if( ch->pcdata->in_mech != '\0')
    {
	send_to_char(C_DEFAULT, "But, you're in a mech!\n\r",ch);
	return;
    }
*/
    rch = get_char( ch );
...
"You can only deposit into your clan bankaccount while you are at your clan head quarters' bank.\n\r",
  ...
  || ( money < 0 ) )
  {
    send_to_char( AT_WHITE, "Invalid ammount of credits.\n\r", ch );
    return;
  }
  else
  {
    ...
    ch->gold -= money;
    sprintf( arg, "You deposit %d credit%s, bringing your total bank balance to %d.\n\r", 
              money, money > 1 ? "s" : "", ch->pcdata->bankaccount );
    send_to_char( AT_WHITE, arg, ch );
    sprintf( arg, "&w$n deposits %d credit%s.", money,
             money > 1 ? "s" : "" );
...
"You must be at your clan head quarters to withdraw money from the clan bankaccount.\n\r", ch );
  ...
  else
  {
    ...
    ch->pcdata->bankaccount -= money;
    ch-> gold += money;
    sprintf( arg, "You withdraw %d credit%s from your account, leaving the balance at %d.\n\r",
             money, money > 1 ? "s" : "", ch->pcdata->bankaccount );
    send_to_char( AT_WHITE, arg, ch );
    sprintf( arg, "&w$n withdraws %d credit%s.", money,
             money > 1 ? "s" : "" );
    act( AT_WHITE, arg, ch, NULL, NULL, TO_ROOM );
    return;
  }
  ...
  if ( ch->gold < cost )
   {
     ...
     sprintf(buf, "That item will cost %d credits, you lack the funds.\n\r", cost );
     send_to_char( AT_WHITE, buf, ch );
     return;
   }
  else
   {
     ...
     sprintf( buf, "You are charged %d credits for repairing %s.\n\r", cost, pObj->short_descr );
     ch->gold -= cost;
     send_to_char ( AT_WHITE, buf, ch );
     pObj->cost = pObj->pIndexData->cost;
     REMOVE_BIT(pObj->extra_flags, ITEM_PATCHED);
...
/*      return; */
    ...
    {
      sprintf( arg, "You have %d credit%s in your account.\n\r",
...
void do_store( CHAR_DATA *ch, char *argument )
{
  ...
  {
    send_to_char( AT_WHITE, 
	 "Storing costs 1000 credits, which you do not have in your bank account.\n\r",
	  ch );
    return;
  }
#ifdef NEW_MONEY
  ...
  send_to_char( AT_WHITE, "The bank deducts 1000 credits from your account.\n\r", ch );
}

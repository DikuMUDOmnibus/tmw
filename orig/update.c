...
void advance_level( CHAR_DATA *ch )
{
	...
	REMOVE_BIT( ch->act, PLR_BOUGHT_PET );
        send_to_char(AT_BLUE, "You gain a class level.\n\r", ch );
        ch->class_level[prime_class(ch)] += 1;
    sprintf( buf, 
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	case COND_THIRST:
		...
		send_to_char(AT_BLOOD, "You need to feed.\n\r", ch );
	    break;
...
void mobile_update( void )
{
	...
	if ( IS_NPC(ch) && (ch->wait -= PULSE_MOBILE) < 0 )
	  ch->wait = 0;
        ...
        if ( IS_NPC(ch) && IS_SET( ch->act, ACT_MUTATION ) &&
             number_percent() < (10 - ch->master->charisma))
           {
           sprintf(buf, "%s gains his own consciousness.\n\r", ch->name );
           send_to_char(AT_WHITE, buf, ch->master );
           stop_follower( ch );
           REMOVE_BIT( ch->act, ACT_MUTATION );
           }
...
/*        sprintf( buf,
            "It is now %s, %s %d%s.",
            day_name[day % 7], month_name[time_info.month],
            day, suf );
        info( buf, 0, 0 ); */
        buf[0] = '\0';
	break;
...
/*		|| IS_AFFECTED( ch, AFF_PEACE )*/
	    ...
	    if ( !IS_SET( mch->act, ACT_AGGRESSIVE )
	      || (IS_SET( mch->act, ACT_WIMPY ) && IS_AWAKE( ch ) )
	      || IS_AFFECTED( ch, AFF_PEACE ) )
/* kjodo      || !IS_SET( mch->act, ACT_KINGDOM */
	      continue;
...
/* kjodo	if ( IS_SET( mch->act, ACT_KINGDOM ) &&
		     mch->kingdom == vch->kingdom &&
		     !IS_SET( mch->act, ACT_AGGRESSIVE ))
			continue;   */
	    ...
	    if ( !victim )
	        continue;
	   if ( IS_AFFECTED3( victim, AFF_IMAGE ))
	      do_image(victim, mch);
	else
 ...
 */
/* hold up... random pulses? tick-timing clients, so what --Kj */
...
void auc_update()
{
...
#else
    sprintf( buf, "%s for %d credits (going ONCE).", auc_obj->short_descr, auc_cost );
#endif
    auc_channel( buf );
...
#else
    sprintf( buf, "%s for %d credits (going TWICE).", auc_obj->short_descr, auc_cost );
#endif
    auc_channel( buf );
    sprintf( buf, "%s auctioning %s.", auc_held->name, auc_obj->name );
    log_string( buf, CHANNEL_GOD, -1 );
...
#else
    sprintf( buf, "%s for %d credits (going THRICE).", auc_obj->short_descr, auc_cost );
#endif
    auc_channel( buf );
    sprintf( buf, "%s auctioning %s.", auc_held->name, auc_obj->name );
    log_string( buf, CHANNEL_GOD, -1 );
  ...
  {
    sprintf( buf, "%s for %d credits SOLD! to %s.", auc_obj->short_descr, auc_cost,
       auc_bid->name );
...
#else
     sprintf( buf, "%d credits not carried for %s, ending auction.", auc_cost, auc_obj->short_descr );
#endif     
